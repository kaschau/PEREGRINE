...
void mixedScaleModel(block_ &b) {
  ...
  //       usg =
  //           1.0 / 3.0 *
  //           (0.25 * b.q(i - 1, j, k, 1) + 0.5 * u + 0.25 * b.q(i + 1, j, k,
  //           1) +
  //            0.25 * b.q(i, j - 1, k, 1) + 0.5 * u + 0.25 * b.q(i, j + 1, k,
  //            1) + 0.25 * b.q(i, j, k - 1, 1) + 0.5 * u + 0.25 * b.q(i, j, k +
  //            1, 1));

  //       vsg =
  //           1.0 / 3.0 *
  //           (0.25 * b.q(i - 1, j, k, 2) + 0.5 * v + 0.25 * b.q(i + 1, j, k,
  //           2) +
  //            0.25 * b.q(i, j - 1, k, 2) + 0.5 * v + 0.25 * b.q(i, j + 1, k,
  //            2) + 0.25 * b.q(i, j, k - 1, 2) + 0.5 * v + 0.25 * b.q(i, j, k +
  //            1, 2));

  //       wsg =
  //           1.0 / 3.0 *
  //           (0.25 * b.q(i - 1, j, k, 3) + 0.5 * w + 0.25 * b.q(i + 1, j, k,
  //           3) +
  //            0.25 * b.q(i, j - 1, k, 3) + 0.5 * w + 0.25 * b.q(i, j + 1, k,
  //            3) + 0.25 * b.q(i, j, k - 1, 3) + 0.5 * w + 0.25 * b.q(i, j, k +
  //            1, 3));

  //       double qc2 =
  //           0.5 * (pow(u - usg, 2.0) + pow(v - vsg, 2.0) + pow(w -
  //           wsg, 2.0));

  //       double delta = pow(b.J(i, j, k), 1.0 / 3.0);

  //       double nusgs = Cm * pow(magSij, alpha) * pow(qc2, (1.0 - alpha)
  //       / 2.0) *
  //                      pow(delta, 1.0 + alpha);
