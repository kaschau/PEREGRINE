  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int i, const int j, const int k) {
        ...
        for (int n = 0; n <= ns - 1; n++) {
          ...
          Tr = T / th.Tcrit(n);
          Tstar = 1.2593 * Tr;
          Omegav = Acoeff * pow(Tstar, -Bcoeff) +
                   Ccoeff * exp(-Dcoeff * Tstar) +
                   Ecoeff * exp(-Fcoeff * Tstar);

          Fc = 1.0 - 0.2756 * th.acentric(n) +
          ...
          G2 = (A1 * (1.0 - exp(-A4 * Yy)) / Yy + A2 * G1 * exp(A5 * Yy) +
                A3 * G1) /
               (A1 * A4 + A2 + A3);

          etaStarStar = A7 * pow(Yy, 2.0) * G2 *
          ...
          alpha = b.qh(i, j, k, 1) * 0.001 * th.MW(n) / b.qh(i, j, k, 0) /
                      (th.Ru / 1000.0) -
                  1.5;
          beta = 0.7862 - 0.7109 * th.acentric(n) +
                 1.3168 * pow(th.acentric(n), 2.0);
          eta0 = 4.0785e-5 * sqrt(th.MW(n) * T) /
          ...
          H2 = (B1 * (1.0 - exp(-B4 * Yy)) / Yy + B2 * G1 * exp(B5 * Yy) +
                B3 * G1) /
               (B1 * B4 + B2 + B3);
