...
void rusanov(block_ b, const thtrdat_ th) {
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int i, const int j, const int k) {
        ...
        double lam = fmax(abs(UL) + b.qh(i, j, k, 3) * b.iS(i, j, k),
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int i, const int j, const int k) {
        ...
        // wave speed estimate / 2
        // double lam = 0.25*( ( b.qh(i,j,k,3) + b.qh(i-1,j,k,3) ) + abs(UR +
        // UL) );
        double lam = fmax(abs(VL) + b.qh(i, j, k, 3) * b.jS(i, j, k),
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int i, const int j, const int k) {
        ...
        // double lam = 0.25*( ( b.qh(i,j,k,3) + b.qh(i-1,j,k,3) ) + abs(UR +
        // UL) );
        double lam = fmax(abs(WL) + b.qh(i, j, k, 3) * b.kS(i, j, k),
