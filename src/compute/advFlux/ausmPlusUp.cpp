...
void ausmPlusUp(block_ &b) {
  ...
  Kokkos::parallel_for(
      "AUSM+UP i face conv fluxes", range_i,
      KOKKOS_LAMBDA(const int i, const int j, const int k) {
        ...
        b.iF(i, j, k, 1) = mDot12 * b.q(i + indx, j, k, 1) * b.iS(i, j, k) +
                           p12 * b.isx(i, j, k);

        // y momentum rho*v*Ui+ p*Ay
        b.iF(i, j, k, 2) = mDot12 * b.q(i + indx, j, k, 2) * b.iS(i, j, k) +
                           p12 * b.isy(i, j, k);

        // w momentum rho*w*Ui+ p*Az
        b.iF(i, j, k, 3) = mDot12 * b.q(i + indx, j, k, 3) * b.iS(i, j, k) +
                           p12 * b.isz(i, j, k);

        // Total energy (rhoE+ p)*Ui)
        b.iF(i, j, k, 4) = mDot12 *
                           (b.Q(i + indx, j, k, 4) + b.q(i + indx, j, k, 0)) /
                           b.Q(i + indx, j, k, 0) * b.iS(i, j, k);

        // Species
        for (int n = 0; n < b.ne - 5; n++) {
          b.iF(i, j, k, 5 + n) =
              mDot12 * b.q(i + indx, j, k, 5 + n) * b.iS(i, j, k);
        }
      });
  ...
  Kokkos::parallel_for(
      "AUSM+UP j face conv fluxes", range_j,
      KOKKOS_LAMBDA(const int i, const int j, const int k) {
        double ufR, vfR, wfR;
        double VR;
        double rhoR, pR;
        ...
        VR = b.jnx(i, j, k) * ufR + b.jny(i, j, k) * vfR + b.jnz(i, j, k) * wfR;
        VL = b.jnx(i, j, k) * ufL + b.jny(i, j, k) * vfL + b.jnz(i, j, k) * wfL;
        ...
        const double Ku = 0.75;
        p12 = p5Plus * pL + p5Minus * pR -
              Ku * p5Plus * p5Minus * (rhoR + rhoL) * (fa * a12) * (VR - VL);
        ...
        // x momentum rho*u*Ui+ p*Ax
        b.jF(i, j, k, 1) = mDot12 * b.q(i, j + indx, k, 1) * b.jS(i, j, k) +
                           p12 * b.jsx(i, j, k);

        // y momentum rho*v*Ui+ p*Ay
        b.jF(i, j, k, 2) = mDot12 * b.q(i, j + indx, k, 2) * b.jS(i, j, k) +
                           p12 * b.jsy(i, j, k);

        // w momentum rho*w*Ui+ p*Az
        b.jF(i, j, k, 3) = mDot12 * b.q(i, j + indx, k, 3) * b.jS(i, j, k) +
                           p12 * b.jsz(i, j, k);

        // Total energy (rhoE+ p)*Ui)
        b.jF(i, j, k, 4) = mDot12 *
                           (b.Q(i, j + indx, k, 4) + b.q(i, j + indx, k, 0)) /
                           b.Q(i, j + indx, k, 0) * b.jS(i, j, k);

        // Species
        for (int n = 0; n < b.ne - 5; n++) {
          b.jF(i, j, k, 5 + n) =
              mDot12 * b.q(i, j + indx, k, 5 + n) * b.jS(i, j, k);
        }
      });
  //-------------------------------------------------------------------------------------------|
  // k flux face range
  ...
  Kokkos::parallel_for(
      "AUSM+UP k face conv fluxes", range_k,
      KOKKOS_LAMBDA(const int i, const int j, const int k) {
        double ufR, vfR, wfR;
        double WR;
        double rhoR, pR;
        ...
        WR = b.knx(i, j, k) * ufR + b.kny(i, j, k) * vfR + b.knz(i, j, k) * wfR;
        WL = b.knx(i, j, k) * ufL + b.kny(i, j, k) * vfL + b.knz(i, j, k) * wfL;
        ...
        const double Ku = 0.75;
        p12 = p5Plus * pL + p5Minus * pR -
              Ku * p5Plus * p5Minus * (rhoR + rhoL) * (fa * a12) * (WR - WL);
        ...
        // x momentum rho*u*Ui+ p*Ax
        b.kF(i, j, k, 1) = mDot12 * b.q(i, j, k + indx, 1) * b.kS(i, j, k) +
                           p12 * b.ksx(i, j, k);

        // y momentum rho*v*Ui+ p*Ay
        b.kF(i, j, k, 2) = mDot12 * b.q(i, j, k + indx, 2) * b.kS(i, j, k) +
                           p12 * b.ksy(i, j, k);

        // w momentum rho*w*Ui+ p*Az
        b.kF(i, j, k, 3) = mDot12 * b.q(i, j, k + indx, 3) * b.kS(i, j, k) +
                           p12 * b.ksz(i, j, k);

        // Total energy (rhoE+ p)*Ui)
        b.kF(i, j, k, 4) = mDot12 *
                           (b.Q(i, j, k + indx, 4) + b.q(i, j, k + indx, 0)) /
                           b.Q(i, j, k + indx, 0) * b.kS(i, j, k);

        // Species
        for (int n = 0; n < b.ne - 5; n++) {
          b.kF(i, j, k, 5 + n) =
              mDot12 * b.q(i, j, k + indx, 5 + n) * b.kS(i, j, k);
        }
      });
}
