...
struct realGasFromPrims {
  ...
  KOKKOS_INLINE_FUNCTION void operator()() const {
    ...
    double cpDep =
        ...
        pow((pow(ZoB, 2.0) + uRG * ZoB + wRG) - (dam / (bm * Ru)) * (ZoB - 1.0),
            2.0) /
...
PG_ABI void pgRealGasFromPrims(const realGasFromPrims &k, const pgTiling &t) {
  forCells("Compute all conserved quantities from primatives via real gas", t,
           k);
}
