...
void chem_GRI30(block_ &b, const thtrdat_ &th, const int &rface /*=0*/,
      ...
      KOKKOS_LAMBDA(const int i, const int j, const int k) {
        ...
        for (int nSub = 0; nSub < nChemSubSteps; nSub++) {
          ...
          // start scope of these temp vars
          {
            ...
            // Reaction #2
            k_f = exp(log(38.7) + 2.7 * logT - (3150.1542797022735 * Tinv));
            ...
            // Reaction #14
            k_f = exp(log(39000000000.00001) - (1781.3971485856307 * Tinv));
            ...
            // Reaction #17
            k_f = exp(log(388.00000000000006) + 2.5 * logT -
                      (1559.9805538461737 * Tinv));
            dG = -gbs[2] + gbs[4] + gbs[18] - gbs[20];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[20];
            q_b = -k_f / K_c * cs[4] * cs[18];
            q[17] = q_f + q_b;

            // Reaction #18
            k_f = exp(log(130.00000000000003) + 2.5 * logT -
                      (2516.097667493829 * Tinv));
            dG = -gbs[2] + gbs[4] + gbs[19] - gbs[20];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[20];
            q_b = -k_f / K_c * cs[4] * cs[19];
            q[18] = q_f + q_b;
            ...
            // Reaction #21
            k_f = exp(log(4.600000000000001e+16) - 1.41 * logT -
                      (14568.205494789268 * Tinv));
            dG = -gbs[2] + gbs[4] + gbs[21] - gbs[22];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[22];
            q_b = -k_f / K_c * cs[4] * cs[21];
            q[21] = q_f + q_b;
            ...
            // Reaction #24
            k_f = exp(log(12500.000000000002) + 1.83 * logT -
                      (110.70829736972846 * Tinv));
            dG = -gbs[2] + gbs[12] + gbs[16] - gbs[24];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[24];
            q_b = -k_f / K_c * cs[12] * cs[16];
            q[24] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[25];
            q_b = -k_f / K_c * cs[12] * cs[17];
            q[25] = q_f + q_b;

            // Reaction #26
            k_f = exp(log(89800.00000000001) + 1.92 * logT -
                      (2863.319145607977 * Tinv));
            ...
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[2] * cs[27];
            q_b = -k_f / K_c * cs[1] * pow(cs[14], 2.0);
            q[27] = q_f + q_b;

            // Reaction #28
            k_f = exp(log(10000000000.000002) - (4025.7562679901257 * Tinv));
            ...
            // Reaction #29
            k_f = exp(log(1750000000.0000002) - (679.3463702233338 * Tinv));
            dG = -gbs[2] + gbs[10] + gbs[15] - gbs[28];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[28];
            q_b = -k_f / K_c * cs[10] * cs[15];
            q[29] = q_f + q_b;

            // Reaction #30
            k_f = exp(log(2500000000.0000005) - (24053.893701241002 * Tinv));
            ...
            // Reaction #31
            k_f = exp(log(100000000000.00002) - (20128.78133995063 * Tinv));
            ...
            // Reaction #37
            k_f = exp(log(26500000000000.004) - 0.6707 * logT -
                      (8575.364070352467 * Tinv));
            ...
            // Reaction #43
            k_f = exp(log(3970000000.0000005) - (337.66030697767184 * Tinv));
            ...
            // Reaction #44
            k_f = exp(log(44800000000.00001) - (537.4384617766818 * Tinv));
            ...
            // Reaction #45
            k_f = exp(log(84000000000.00002) - (319.54440377171625 * Tinv));
            ...
            // Reaction #47
            k_f = exp(log(10000000000.000002) - (1811.5903205955567 * Tinv));
            ...
            // Reaction #51
            k_f = exp(log(13900000000000.002) - 0.534 * logT -
            ...
            k_f = exp(log(660000.0000000001) + 1.62 * logT -
                      (5454.899743126621 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[12] - gbs[13];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[13];
            q_b = -k_f / K_c * cs[0] * cs[12];
            q[52] = q_f + q_b;
            ...
            // Reaction #57
            k_f = exp(log(57400.000000000015) + 1.9 * logT -
                      (1379.8279608536157 * Tinv));
            ...
            // Reaction #58
            k_f = exp(log(1055000000.0000002) + 0.5 * logT -
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[18];
            q_b = -k_f / K_c * cs[0] * cs[17];
            q[59] = q_f + q_b;

            // Reaction #60
            k_f = exp(log(165000000.00000003) + 0.65 * logT -
                      (-142.91434751364946 * Tinv));
            ...
            // Reaction #61
            k_f = exp(log(32800000000.000004) - 0.09 * logT -
                      (306.9639154342471 * Tinv));
            ...
            // Reaction #62
            k_f = exp(log(2430000000.0000005) + 0.515 * logT -
            ...
            // Reaction #63
            k_f = exp(log(41500.00000000001) + 1.63 * logT -
                      (968.1943824516253 * Tinv));
            dG = gbs[18] - gbs[19];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[19];
            ...
            // Reaction #65
            k_f = exp(log(1500000000.0000002) + 0.5 * logT -
                      (-55.35414868486423 * Tinv));
            ...
            // Reaction #66
            k_f = exp(log(262000000000.00003) - 0.23 * logT -
                      (538.4449008436793 * Tinv));
            ...
            // Reaction #67
            k_f = exp(log(17000.000000000004) + 2.1 * logT -
                      (2450.679128138989 * Tinv));
            ...
            // Reaction #68
            k_f = exp(log(4200.000000000001) + 2.1 * logT -
                      (2450.679128138989 * Tinv));
            ...
            // Reaction #74
            k_f = exp(log(1325.0000000000002) + 2.53 * logT -
                      (6159.407090024893 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[23] - gbs[24];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[24];
            q_b = -k_f / K_c * cs[0] * cs[23];
            q[74] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[27];
            q_b = -k_f / K_c * cs[11] * cs[14];
            q[78] = q_f + q_b;

            // Reaction #79
            k_f = exp(log(50000000000.00001) - (4025.7562679901257 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[27] - gbs[28];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[28];
            q_b = -k_f / K_c * cs[0] * cs[27];
            q[79] = q_f + q_b;

            // Reaction #80
            k_f = exp(log(11300000000.000002) - (1725.036560833769 * Tinv));
            dG = -gbs[1] + gbs[12] + gbs[14] - gbs[28];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[28];
            q_b = -k_f / K_c * cs[12] * cs[14];
            q[80] = q_f + q_b;
            ...
            // Reaction #85
            k_f = exp(log(35.7) + 2.4 * logT - (-1061.7932156823956 * Tinv));
            ...
            // Reaction #86
            k_f = exp(log(14500000000.000002) - (-251.60976674938286 * Tinv));
            ...
            // Reaction #87
            k_f = exp(log(2000000000.0000002) - (214.87474080397297 * Tinv));
            ...
            // Reaction #88
            k_f = exp(log(1700000000000000.2) - (14799.686480198701 * Tinv));
            dG = -gbs[4] + gbs[5] + gbs[6] - gbs[7];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[7];
            q_b = -k_f / K_c * cs[5] * cs[6];
            ...
            // Reaction #94
            k_f = exp(log(2790000000000000.5) - 1.43 * logT -
            ...
            // Reaction #96
            k_f = exp(log(644000000000000.1) - 1.34 * logT -
                      (713.0620789677511 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[16];
            q_b = -k_f / K_c * cs[5] * cs[14];
            q[99] = q_f + q_b;

            // Reaction #100
            k_f = exp(log(3430000.0000000005) + 1.18 * logT -
                      (-224.9391314739483 * Tinv));
            ...
            // Reaction #107
            k_f = exp(log(504.0000000000001) + 2.3 * logT -
                      (6793.463702233337 * Tinv));
            dG = gbs[1] - gbs[4] - gbs[22] + gbs[29];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[22];
            q_b = -k_f / K_c * cs[1] * cs[29];
            q[107] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[24];
            q_b = -k_f / K_c * cs[5] * cs[23];
            q[111] = q_f + q_b;

            // Reaction #112
            k_f = exp(log(3540.0000000000005) + 2.12 * logT -
                      (437.8009941439262 * Tinv));
            ...
            // Reaction #113
            k_f = exp(log(7500000000.000001) - (1006.4390669975314 * Tinv));
            dG = -gbs[4] + gbs[5] + gbs[27] - gbs[28];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[28];
            q_b = -k_f / K_c * cs[5] * cs[27];
            q[113] = q_f + q_b;

            // Reaction #114
            k_f = exp(log(130000000.00000001) - (-820.2478396029882 * Tinv));
            ...
            // Reaction #115
            k_f = exp(log(420000000000.00006) - (6038.634401985189 * Tinv));
            dG = gbs[3] - 2.0 * gbs[6] + gbs[7];
            K_c = exp(-dG);
            q_f = k_f * pow(cs[6], 2.0);
            q_b = -k_f / K_c * cs[3] * cs[7];
            ...
            // Reaction #119
            k_f = exp(log(150000000000.00003) - (11875.980990570872 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[6] * cs[17];
            q_b = -k_f / K_c * cs[7] * cs[16];
            q[120] = q_f + q_b;

            // Reaction #121
            k_f = exp(log(58000000000.00001) - (289.85445129528904 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[9];
            q_b = -k_f / K_c * cs[2] * cs[16];
            q[124] = q_f + q_b;

            // Reaction #125
            k_f = exp(log(108000000000.00002) - (1565.0127491811616 * Tinv));
            ...
            // Reaction #126
            k_f = exp(log(5710000000.000001) - (-379.93074779156814 * Tinv));
            ...
            // Reaction #131
            k_f = exp(log(190000000000.00003) - (7946.842873012509 * Tinv));
            dG = -gbs[9] + gbs[14] - gbs[15] + gbs[16];
            K_c = exp(-dG);
            q_f = k_f * cs[9] * cs[15];
            q_b = -k_f / K_c * cs[14] * cs[16];
            q[131] = q_f + q_b;

            // Reaction #132
            k_f = exp(log(94600000000.00002) - (-259.15805975186436 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[9] * cs[27];
            q_b = -k_f / K_c * cs[14] * cs[22];
            q[133] = q_f + q_b;

            // Reaction #134
            k_f = exp(log(5000000000.000001) - (754.8293002481486 * Tinv));
            dG = gbs[1] - gbs[3] + gbs[4] - gbs[10] + gbs[14];
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[3] * cs[10];
            q_b = -k_f / K_c * cs[1] * cs[4] * cs[14];
            q[134] = q_f;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[0] * cs[10];
            q_b = -k_f / K_c * cs[1] * cs[12];
            q[135] = q_f + q_b;

            // Reaction #136
            k_f = exp(log(1600000000000.0002) - (6010.454108109258 * Tinv));
            dG = gbs[0] - 2.0 * gbs[10] + gbs[22];
            K_c = exp(-dG);
            q_f = k_f * pow(cs[10], 2.0);
            q_b = -k_f / K_c * cs[0] * cs[22];
            q[136] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[10] * cs[27];
            q_b = -k_f / K_c * cs[14] * cs[23];
            q[140] = q_f + q_b;

            // Reaction #141
            k_f = exp(log(15000000000.000002) - (301.93172009925945 * Tinv));
            dG = gbs[10] - gbs[11];
            K_c = exp(-dG);
            q_f = k_f * cs[11] * cs[47];
            q_b = -k_f / K_c * cs[10] * cs[47];
            q[141] = q_f + q_b;

            // Reaction #142
            k_f = exp(log(9000000000.000002) - (301.93172009925945 * Tinv));
            dG = gbs[10] - gbs[11];
            K_c = exp(-dG);
            q_f = k_f * cs[11] * cs[48];
            q_b = -k_f / K_c * cs[10] * cs[48];
            q[142] = q_f + q_b;
            ...
            // Reaction #146
            k_f = exp(log(482000000000000.06) - 1.16 * logT -
            ...
            // Reaction #148
            k_f = exp(log(12000000000.000002) - (-286.83513409429645 * Tinv));
            ...
            // Reaction #149
            k_f = exp(log(16000000000.000002) - (-286.83513409429645 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[11] * cs[15];
            q_b = -k_f / K_c * cs[14] * cs[17];
            q[152] = q_f + q_b;

            // Reaction #153
            k_f = exp(log(40000000000.00001) - (-276.77074342432115 * Tinv));
            dG = -gbs[11] + gbs[12] + gbs[25] - gbs[26];
            K_c = exp(-dG);
            q_f = k_f * cs[11] * cs[26];
            q_b = -k_f / K_c * cs[12] * cs[25];
            q[153] = q_f + q_b;

            // Reaction #154
            k_f = exp(log(35600000000.00001) - (15338.13138104238 * Tinv));
            dG = gbs[2] - gbs[3] - gbs[12] + gbs[19];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[12];
            q_b = -k_f / K_c * cs[2] * cs[19];
            q[154] = q_f + q_b;

            // Reaction #155
            k_f = exp(log(2310000000.0000005) - (10222.904823027426 * Tinv));
            ...
            // Reaction #157
            k_f = exp(log(67700000000000.01) - 1.18 * logT -
            ...
            // Reaction #158
            k_f = exp(log(6840000000.000001) + 0.1 * logT -
                      (5334.127055086917 * Tinv));
            ...
            // Reaction #161
            k_f = exp(log(30000.000000000004) + 1.5 * logT -
                      (5002.002162977731 * Tinv));
            ...
            // Reaction #162
            k_f = exp(log(10000.000000000002) + 1.5 * logT -
                      (5002.002162977731 * Tinv));
            dG = -gbs[12] + gbs[13] + gbs[19] - gbs[20];
            K_c = exp(-dG);
            q_f = k_f * cs[12] * cs[20];
            q_b = -k_f / K_c * cs[13] * cs[19];
            q[162] = q_f + q_b;
            ...
            k_f = exp(log(6140.000000000002) + 1.74 * logT -
                      (5258.644125062102 * Tinv));
            dG = -gbs[12] + gbs[13] + gbs[25] - gbs[26];
            K_c = exp(-dG);
            q_f = k_f * cs[12] * cs[26];
            q_b = -k_f / K_c * cs[13] * cs[25];
            q[164] = q_f + q_b;
            ...
            // Reaction #167
            k_f = exp(log(13450000000.000002) - (201.2878133995063 * Tinv));
            dG = -gbs[3] + gbs[6] + gbs[14] - gbs[16];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[16];
            q_b = -k_f / K_c * cs[6] * cs[14];
            q[167] = q_f + q_b;

            // Reaction #168
            k_f = exp(log(18000000000.000004) - (452.8975801488892 * Tinv));
            dG = -gbs[3] + gbs[6] + gbs[17] - gbs[18];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[18];
            q_b = -k_f / K_c * cs[6] * cs[17];
            q[168] = q_f + q_b;

            // Reaction #169
            k_f = exp(log(4.2800000000000005e-16) + 7.6 * logT -
                      (-1776.364953250643 * Tinv));
            ...
            // Reaction #170
            k_f = exp(log(10000000000.000002) - (-379.93074779156814 * Tinv));
            ...
            // Reaction #171
            k_f = exp(log(56800000.00000001) + 0.9 * logT -
                      (1002.9165302630402 * Tinv));
            ...
            // Reaction #172
            k_f = exp(log(45800000000000.01) - 1.39 * logT -
                      (510.7678265012472 * Tinv));
            dG = -gbs[3] + gbs[16] + gbs[17] - gbs[23];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[23];
            q_b = -k_f / K_c * cs[16] * cs[17];
            q[172] = q_f + q_b;

            // Reaction #173
            k_f = exp(log(8000000000000.0) + 0.44 * logT -
            ...
            // Reaction #174
            k_f = exp(log(840000000.0000001) - (1949.9756923077173 * Tinv));
            dG = -gbs[3] + gbs[6] + gbs[24] - gbs[25];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[25];
            q_b = -k_f / K_c * cs[6] * cs[24];
            q[174] = q_f + q_b;

            // Reaction #175
            k_f = exp(log(3200000000.0000005) - (429.74948160794594 * Tinv));
            dG = -gbs[3] + gbs[4] + 2.0 * gbs[14] - gbs[27];
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[3] * cs[27];
            q_b = -k_f / K_c * cs[4] * pow(cs[14], 2.0);
            q[175] = q_f + q_b;
            ...
            K_c = prefRuT * exp(-dG);
            q_f = k_f * pow(cs[27], 2.0);
            q_b = -k_f / K_c * pow(cs[14], 2.0) * cs[22];
            q[176] = q_f + q_b;

            // Reaction #177
            k_f = exp(log(27000000000.000004) - (178.64293439206185 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[30];
            q_b = -k_f / K_c * cs[2] * cs[35];
            q[178] = q_f + q_b;

            // Reaction #179
            k_f = exp(log(33600000000.000008) - (193.73952039702482 * Tinv));
            dG = gbs[1] - gbs[4] - gbs[30] + gbs[35];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[30];
            q_b = -k_f / K_c * cs[1] * cs[35];
            q[179] = q_f + q_b;

            // Reaction #180
            k_f = exp(log(1400000000.0000002) - (5439.803157121658 * Tinv));
            dG = -gbs[2] + gbs[3] - gbs[37] + gbs[47];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[37];
            q_b = -k_f / K_c * cs[3] * cs[47];
            q[180] = q_f + q_b;

            // Reaction #181
            k_f = exp(log(29000000000.000004) - (11649.532200496427 * Tinv));
            dG = -gbs[2] + 2.0 * gbs[35] - gbs[37];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[37];
            q_b = -k_f / K_c * pow(cs[35], 2.0);
            q[181] = q_f + q_b;

            // Reaction #182
            k_f = exp(log(387000000000.00006) - (9500.784792456698 * Tinv));
            dG = -gbs[1] + gbs[4] - gbs[37] + gbs[47];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[37];
            q_b = -k_f / K_c * cs[4] * cs[47];
            q[182] = q_f + q_b;

            // Reaction #183
            k_f = exp(log(2000000000.0000002) - (10597.803375484007 * Tinv));
            dG = -gbs[4] + gbs[6] - gbs[37] + gbs[47];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[37];
            q_b = -k_f / K_c * cs[6] * cs[47];
            q[183] = q_f + q_b;
            ...
            // Reaction #185
            k_f = exp(log(2110000000.0000005) - (-241.54537607940756 * Tinv));
            dG = gbs[4] - gbs[6] - gbs[35] + gbs[36];
            K_c = exp(-dG);
            q_f = k_f * cs[6] * cs[35];
            q_b = -k_f / K_c * cs[4] * cs[36];
            q[185] = q_f + q_b;
            ...
            // Reaction #187
            k_f = exp(log(3900000000.0000005) - (-120.77268803970378 * Tinv));
            dG = -gbs[2] + gbs[3] + gbs[35] - gbs[36];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[36];
            q_b = -k_f / K_c * cs[3] * cs[35];
            q[187] = q_f + q_b;

            // Reaction #188
            k_f = exp(log(132000000000.00002) - (181.15903205955567 * Tinv));
            dG = -gbs[1] + gbs[4] + gbs[35] - gbs[36];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[36];
            q_b = -k_f / K_c * cs[4] * cs[35];
            q[188] = q_f + q_b;
            ...
            // Reaction #190
            k_f = exp(log(32000000000.000004) - (166.0624460545927 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[30] - gbs[31];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[31];
            q_b = -k_f / K_c * cs[0] * cs[30];
            q[190] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[31];
            q_b = -k_f / K_c * cs[2] * cs[38];
            q[193] = q_f + q_b;

            // Reaction #194
            k_f = exp(log(1280.0000000000002) + 1.5 * logT -
                      (50.32195334987657 * Tinv));
            ...
            // Reaction #196
            k_f = exp(log(20000000000.000004) - (6969.590538957906 * Tinv));
            dG = gbs[0] - gbs[5] - gbs[31] + gbs[38];
            K_c = exp(-dG);
            q_f = k_f * cs[5] * cs[31];
            q_b = -k_f / K_c * cs[0] * cs[38];
            q[196] = q_f + q_b;
            ...
            // Reaction #201
            k_f = exp(log(40000000000.00001) - (1836.751297270495 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[31] - gbs[32];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[32];
            q_b = -k_f / K_c * cs[0] * cs[31];
            q[201] = q_f + q_b;

            // Reaction #202
            k_f = exp(log(90000.00000000001) + 1.5 * logT -
                      (-231.48098540943224 * Tinv));
            dG = -gbs[4] + gbs[5] + gbs[31] - gbs[32];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[32];
            q_b = -k_f / K_c * cs[5] * cs[31];
            q[202] = q_f + q_b;
            ...
            // Reaction #204
            k_f = exp(log(130000000000.00002) - 0.11 * logT -
                      (2506.033276823853 * Tinv));
            dG = gbs[1] - gbs[34] + gbs[47];
            K_c = prefRuT * exp(-dG);
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[12] * cs[34];
            q_b = -k_f / K_c * cs[13] * cs[47];
            q[210] = q_f + q_b;

            // Reaction #211
            k_f = exp(log(44800000000000.01) - 1.32 * logT -
            ...
            // Reaction #213
            k_f = exp(log(900000000.0000001) + 0.72 * logT -
                      (332.1248921091854 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[35] - gbs[38];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[38];
            q_b = -k_f / K_c * cs[0] * cs[35];
            q[213] = q_f + q_b;

            // Reaction #214
            k_f = exp(log(13000.000000000002) + 1.9 * logT -
                      (-478.0585568238275 * Tinv));
            dG = -gbs[4] + gbs[5] + gbs[35] - gbs[38];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[38];
            q_b = -k_f / K_c * cs[5] * cs[35];
            q[214] = q_f + q_b;

            // Reaction #215
            k_f = exp(log(10000000000.000002) - (6541.853935483955 * Tinv));
            dG = -gbs[3] + gbs[6] + gbs[35] - gbs[38];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[38];
            q_b = -k_f / K_c * cs[6] * cs[35];
            q[215] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[39];
            q_b = -k_f / K_c * cs[1] * cs[46];
            q[217] = q_f + q_b;

            // Reaction #218
            k_f = exp(log(8000000000.000001) - (3754.0177199007926 * Tinv));
            dG = gbs[4] - gbs[5] - gbs[39] + gbs[40];
            K_c = exp(-dG);
            q_f = k_f * cs[5] * cs[39];
            q_b = -k_f / K_c * cs[4] * cs[40];
            q[218] = q_f + q_b;

            // Reaction #219
            k_f = exp(log(6140000000.000001) - (-221.4165947394569 * Tinv));
            dG = gbs[2] - gbs[3] - gbs[39] + gbs[46];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[39];
            q_b = -k_f / K_c * cs[2] * cs[46];
            q[219] = q_f + q_b;

            // Reaction #220
            k_f = exp(log(295.00000000000006) + 2.45 * logT -
                      (1127.2117550372352 * Tinv));
            dG = -gbs[0] + gbs[1] - gbs[39] + gbs[40];
            K_c = exp(-dG);
            q_f = k_f * cs[0] * cs[39];
            q_b = -k_f / K_c * cs[1] * cs[40];
            q[220] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[30] * cs[46];
            q_b = -k_f / K_c * cs[14] * cs[47];
            q[224] = q_f + q_b;

            // Reaction #225
            k_f = exp(log(2000000000.0000002) - (10064.390669975315 * Tinv));
            dG = -gbs[3] + gbs[15] + gbs[35] - gbs[46];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[46];
            q_b = -k_f / K_c * cs[15] * cs[35];
            q[225] = q_f + q_b;

            // Reaction #226
            k_f = exp(log(310000000000.00006) - (27199.015785608288 * Tinv));
            ...
            // Reaction #227
            k_f = exp(log(190000000000000.03) - 1.52 * logT -
                      (372.38245478908664 * Tinv));
            dG = gbs[14] - gbs[35] + gbs[37] - gbs[46];
            K_c = exp(-dG);
            q_f = k_f * cs[35] * cs[46];
            q_b = -k_f / K_c * cs[14] * cs[37];
            q[227] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[35] * cs[46];
            q_b = -k_f / K_c * cs[15] * cs[47];
            q[228] = q_f + q_b;

            // Reaction #229
            k_f = exp(log(1.0400000000000003e+26) - 3.3 * logT -
                      (63707.592940943745 * Tinv));
            ...
            // Reaction #230
            k_f = exp(log(20.3) + 2.64 * logT - (2506.033276823853 * Tinv));
            ...
            // Reaction #231
            k_f = exp(log(5.07) + 2.64 * logT - (2506.033276823853 * Tinv));
            ...
            // Reaction #232
            k_f = exp(log(3910000.0000000005) + 1.58 * logT -
                      (13385.639591067169 * Tinv));
            dG = -gbs[2] + gbs[4] + gbs[39] - gbs[40];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[40];
            q_b = -k_f / K_c * cs[4] * cs[39];
            q[232] = q_f + q_b;

            // Reaction #233
            k_f = exp(log(1100.0) + 2.03 * logT - (6728.045162878498 * Tinv));
            dG = gbs[1] - gbs[4] - gbs[40] + gbs[44];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[40];
            q_b = -k_f / K_c * cs[1] * cs[44];
            q[233] = q_f + q_b;

            // Reaction #234
            k_f = exp(log(4.400000000000001) + 2.26 * logT -
                      (3220.6050143921007 * Tinv));
            dG = gbs[1] - gbs[4] - gbs[40] + gbs[45];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[40];
            q_b = -k_f / K_c * cs[1] * cs[45];
            q[234] = q_f + q_b;

            // Reaction #235
            k_f = exp(log(0.16000000000000003) + 2.56 * logT -
                      (4528.975801488891 * Tinv));
            dG = -gbs[4] + gbs[14] + gbs[32] - gbs[40];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[40];
            q_b = -k_f / K_c * cs[14] * cs[32];
            q[235] = q_f + q_b;
            ...
            // Reaction #237
            k_f = exp(log(60000000000.00001) - (201.2878133995063 * Tinv));
            ...
            // Reaction #238
            k_f = exp(log(63000000000.00001) - (23158.1629316132 * Tinv));
            dG = -gbs[8] + gbs[30] + gbs[39] - gbs[47];
            K_c = exp(-dG);
            q_f = k_f * cs[8] * cs[47];
            q_b = -k_f / K_c * cs[30] * cs[39];
            q[238] = q_f + q_b;

            // Reaction #239
            k_f = exp(log(3120000.0000000005) + 0.88 * logT -
                      (10129.809209330155 * Tinv));
            dG = -gbs[9] + gbs[30] + gbs[40] - gbs[47];
            K_c = exp(-dG);
            q_f = k_f * cs[9] * cs[47];
            q_b = -k_f / K_c * cs[30] * cs[40];
            q[239] = q_f + q_b;
            ...
            // Reaction #241
            k_f = exp(log(10000000000.000002) - (37238.24547890866 * Tinv));
            dG = -gbs[10] + gbs[31] + gbs[40] - gbs[47];
            K_c = exp(-dG);
            q_f = k_f * cs[10] * cs[47];
            q_b = -k_f / K_c * cs[31] * cs[40];
            q[241] = q_f + q_b;

            // Reaction #242
            k_f = exp(log(100000000.00000001) - (32709.269677419772 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[9] * cs[35];
            q_b = -k_f / K_c * cs[16] * cs[30];
            q[247] = q_f + q_b;

            // Reaction #248
            k_f = exp(log(310000000000000.06) - 1.38 * logT -
                      (639.0888075434325 * Tinv));
            ...
            // Reaction #249
            k_f = exp(log(290000000000.00006) - 0.69 * logT -
                      (382.44684545906193 * Tinv));
            ...
            // Reaction #250
            k_f = exp(log(38000000000.00001) - 0.36 * logT -
                      (291.86732942928415 * Tinv));
            dG = gbs[1] - gbs[10] - gbs[35] + gbs[43];
            K_c = exp(-dG);
            q_f = k_f * cs[10] * cs[35];
            q_b = -k_f / K_c * cs[1] * cs[43];
            q[250] = q_f + q_b;
            ...
            // Reaction #254
            k_f = exp(log(96000000000.00002) - (14492.722564764454 * Tinv));
            dG = gbs[5] - gbs[12] - gbs[35] + gbs[40];
            K_c = exp(-dG);
            q_f = k_f * cs[12] * cs[35];
            q_b = -k_f / K_c * cs[5] * cs[40];
            q[254] = q_f + q_b;

            // Reaction #255
            k_f = exp(log(1000000000.0000001) - (10945.024853598155 * Tinv));
            dG = gbs[4] - gbs[12] - gbs[35] + gbs[41];
            K_c = exp(-dG);
            q_f = k_f * cs[12] * cs[35];
            q_b = -k_f / K_c * cs[4] * cs[41];
            q[255] = q_f + q_b;
            ...
            // Reaction #261
            k_f = exp(log(98000.00000000001) + 1.41 * logT -
                      (4277.366034739509 * Tinv));
            dG = -gbs[2] + gbs[15] + gbs[31] - gbs[45];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[45];
            q_b = -k_f / K_c * cs[15] * cs[31];
            q[261] = q_f + q_b;

            // Reaction #262
            k_f = exp(log(150000.00000000003) + 1.57 * logT -
                      (22141.659473945692 * Tinv));
            dG = -gbs[2] + gbs[14] + gbs[38] - gbs[45];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[45];
            q_b = -k_f / K_c * cs[14] * cs[38];
            q[262] = q_f + q_b;

            // Reaction #263
            k_f = exp(log(2200.0) + 2.11 * logT - (5736.702681885929 * Tinv));
            dG = -gbs[2] + gbs[4] - gbs[45] + gbs[46];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[45];
            q_b = -k_f / K_c * cs[4] * cs[46];
            q[263] = q_f + q_b;

            // Reaction #264
            k_f = exp(log(22500.000000000004) + 1.7 * logT -
                      (1912.23422729531 * Tinv));
            ...
            // Reaction #265
            k_f = exp(log(105.00000000000003) + 2.5 * logT -
                      (6692.8197955335845 * Tinv));
            dG = gbs[0] - gbs[1] - gbs[45] + gbs[46];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[45];
            q_b = -k_f / K_c * cs[0] * cs[46];
            q[265] = q_f + q_b;

            // Reaction #266
            k_f = exp(log(33000.00000000001) + 1.5 * logT -
                      (1811.5903205955567 * Tinv));
            dG = -gbs[4] + gbs[5] - gbs[45] + gbs[46];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[45];
            q_b = -k_f / K_c * cs[5] * cs[46];
            q[266] = q_f + q_b;

            // Reaction #267
            k_f = exp(log(3300.000000000001) + 1.5 * logT -
                      (1811.5903205955567 * Tinv));
            dG = -gbs[4] + gbs[15] + gbs[32] - gbs[45];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[45];
            q_b = -k_f / K_c * cs[15] * cs[32];
            q[267] = q_f + q_b;

            // Reaction #268
            k_f = exp(log(11800000000000.002) - (42632.75887801543 * Tinv));
            ...
            // Reaction #269
            k_f = exp(log(2100000000000.0002) - 0.69 * logT -
                      (1434.1756704714824 * Tinv));
            dG = -gbs[43] + gbs[45];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[43];
            q_b = -k_f / K_c * cs[1] * cs[45];
            ...
            // Reaction #270
            k_f = exp(log(270000000.00000006) + 0.18 * logT -
                      (1066.8254110173834 * Tinv));
            ...
            // Reaction #271
            k_f = exp(log(170000000000.00003) - 0.75 * logT -
                      (1454.304451811433 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[27] * cs[35];
            q_b = -k_f / K_c * cs[14] * cs[43];
            q[273] = q_f + q_b;

            // Reaction #274
            k_f = exp(log(610000000000.0001) - 0.31 * logT -
                      (145.93366471464208 * Tinv));
            dG = gbs[1] - gbs[12] - gbs[30] + gbs[41];
            K_c = exp(-dG);
            q_f = k_f * cs[12] * cs[30];
            q_b = -k_f / K_c * cs[1] * cs[41];
            q[274] = q_f + q_b;

            // Reaction #275
            k_f = exp(log(3700000000.0000005) + 0.15 * logT -
                      (-45.28975801488892 * Tinv));
            dG = gbs[0] - gbs[12] - gbs[30] + gbs[40];
            K_c = exp(-dG);
            q_f = k_f * cs[12] * cs[30];
            q_b = -k_f / K_c * cs[0] * cs[40];
            q[275] = q_f + q_b;

            // Reaction #276
            k_f = exp(log(540.0) + 2.4 * logT - (4989.421674640263 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[32] - gbs[33];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[33];
            q_b = -k_f / K_c * cs[0] * cs[32];
            q[276] = q_f + q_b;

            // Reaction #277
            k_f = exp(log(50000.00000000001) + 1.6 * logT -
                      (480.57465449132127 * Tinv));
            dG = -gbs[4] + gbs[5] + gbs[32] - gbs[33];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[33];
            q_b = -k_f / K_c * cs[5] * cs[32];
            q[277] = q_f + q_b;

            // Reaction #278
            k_f = exp(log(9400.000000000002) + 1.94 * logT -
                      (3250.7981864020267 * Tinv));
            dG = -gbs[2] + gbs[4] + gbs[32] - gbs[33];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[33];
            q_b = -k_f / K_c * cs[4] * cs[32];
            q[278] = q_f + q_b;

            // Reaction #279
            k_f = exp(log(10000000000.000002) - (7221.200305707288 * Tinv));
            ...
            // Reaction #280
            k_f = exp(log(6160000000000.001) - 0.752 * logT -
                      (173.61073905707417 * Tinv));
            dG = gbs[35] - gbs[36] - gbs[39] + gbs[46];
            K_c = exp(-dG);
            q_f = k_f * cs[36] * cs[39];
            q_b = -k_f / K_c * cs[35] * cs[46];
            q[280] = q_f + q_b;

            // Reaction #281
            k_f = exp(log(3250000000.0000005) - (-354.76977111662984 * Tinv));
            dG = gbs[15] - gbs[36] + gbs[37] - gbs[46];
            K_c = exp(-dG);
            q_f = k_f * cs[36] * cs[46];
            q_b = -k_f / K_c * cs[15] * cs[37];
            q[281] = q_f + q_b;

            // Reaction #282
            k_f = exp(log(3000000000.0000005) - (5686.380728536053 * Tinv));
            ...
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[2] * cs[12];
            q_b = -k_f / K_c * cs[0] * cs[1] * cs[14];
            q[283] = q_f;

            // Reaction #284
            k_f = exp(log(6700.000000000001) + 1.83 * logT -
                      (110.70829736972846 * Tinv));
            dG = gbs[1] - gbs[2] - gbs[24] + gbs[51];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[24];
            q_b = -k_f / K_c * cs[1] * cs[51];
            q[284] = q_f + q_b;
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[25];
            q_b = -k_f / K_c * cs[1] * cs[52];
            q[285] = q_f + q_b;

            // Reaction #286
            k_f = exp(log(5000000000000.001) - (8720.794515533611 * Tinv));
            dG = gbs[3] - gbs[4] + gbs[5] - gbs[6];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[6];
            q_b = -k_f / K_c * cs[3] * cs[5];
            ...
            // Reaction #287
            k_f = exp(log(8000000.000000001) + 0.5 * logT -
            ...
            // Reaction #288
            k_f = exp(log(1970000000.0000002) + 0.43 * logT -
            ...
            // Reaction #289
            k_f = exp(log(5800000000.000001) - (754.8293002481486 * Tinv));
            dG = 2.0 * gbs[1] - gbs[3] - gbs[10] + gbs[15];
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[3] * cs[10];
            q_b = -k_f / K_c * pow(cs[1], 2.0) * cs[15];
            q[289] = q_f;

            // Reaction #290
            k_f = exp(log(2400000000.0000005) - (754.8293002481486 * Tinv));
            ...
            // Reaction #291
            k_f = exp(log(200000000000.00003) - (5529.879453617937 * Tinv));
            dG = 2.0 * gbs[1] - 2.0 * gbs[10] + gbs[22];
            K_c = prefRuT * exp(-dG);
            q_f = k_f * pow(cs[10], 2.0);
            q_b = -k_f / K_c * pow(cs[1], 2.0) * cs[22];
            q[291] = q_f;

            // Reaction #292
            k_f = exp(log(68200000.00000001) + 0.25 * logT -
                      (-470.510263821346 * Tinv));
            ...
            // Reaction #293
            k_f = exp(log(303000000.00000006) + 0.29 * logT -
                      (5.5354148684864235 * Tinv));
            dG = gbs[2] - gbs[3] - gbs[23] + gbs[51];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[23];
            q_b = -k_f / K_c * cs[2] * cs[51];
            q[293] = q_f + q_b;

            // Reaction #294
            k_f = exp(log(1337.0000000000002) + 1.61 * logT -
                      (-193.23630086352605 * Tinv));
            dG = -gbs[3] + gbs[6] + gbs[22] - gbs[23];
            K_c = exp(-dG);
            q_f = k_f * cs[3] * cs[23];
            q_b = -k_f / K_c * cs[6] * cs[22];
            q[294] = q_f + q_b;

            // Reaction #295
            k_f = exp(log(2920000000.0000005) - (909.8209165657685 * Tinv));
            dG = -gbs[2] + gbs[4] + gbs[51] - gbs[52];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[52];
            q_b = -k_f / K_c * cs[4] * cs[51];
            q[295] = q_f + q_b;
            ...
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[2] * cs[52];
            q_b = -k_f / K_c * cs[4] * cs[12] * cs[14];
            q[296] = q_f;

            // Reaction #297
            k_f = exp(log(30100000000.000004) - (19701.044736476677 * Tinv));
            dG = -gbs[3] + gbs[6] + gbs[12] + gbs[14] - gbs[52];
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[3] * cs[52];
            q_b = -k_f / K_c * cs[6] * cs[12] * cs[14];
            q[297] = q_f;

            // Reaction #298
            k_f = exp(log(2050000.0000000005) + 1.16 * logT -
                      (1210.2429780645316 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[51] - gbs[52];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[52];
            q_b = -k_f / K_c * cs[0] * cs[51];
            q[298] = q_f + q_b;
            ...
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[1] * cs[52];
            q_b = -k_f / K_c * cs[0] * cs[12] * cs[14];
            q[299] = q_f;

            // Reaction #300
            k_f = exp(log(23430000.000000004) + 0.73 * logT -
                      (-560.0833407841262 * Tinv));
            dG = -gbs[4] + gbs[5] + gbs[12] + gbs[14] - gbs[52];
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[4] * cs[52];
            q_b = -k_f / K_c * cs[5] * cs[12] * cs[14];
            q[300] = q_f;

            // Reaction #301
            k_f = exp(log(3010000000.0000005) - (5999.8864979057835 * Tinv));
            dG = -gbs[6] + gbs[7] + gbs[12] + gbs[14] - gbs[52];
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[6] * cs[52];
            q_b = -k_f / K_c * cs[7] * cs[12] * cs[14];
            q[301] = q_f;

            // Reaction #302
            k_f = exp(log(2720.0000000000005) + 1.77 * logT -
                      (2979.059638312693 * Tinv));
            ...
            // Reaction #303
            k_f = exp(log(486500000.00000006) + 0.422 * logT -
                      (-883.1502812903339 * Tinv));
            ...
            K_c = prefRuT * exp(-dG);
            q_f = k_f * cs[3] * cs[51];
            q_b = -k_f / K_c * cs[4] * pow(cs[16], 2.0);
            q[306] = q_f;
            ...
            // Reaction #312
            k_f = exp(log(193.00000000000003) + 2.68 * logT -
                      (1869.9637864814135 * Tinv));
            dG = -gbs[2] + gbs[4] + gbs[49] - gbs[50];
            K_c = exp(-dG);
            q_f = k_f * cs[2] * cs[50];
            q_b = -k_f / K_c * cs[4] * cs[49];
            q[312] = q_f + q_b;

            // Reaction #313
            k_f = exp(log(1320.0000000000002) + 2.54 * logT -
                      (3399.7511683176613 * Tinv));
            dG = gbs[0] - gbs[1] + gbs[49] - gbs[50];
            K_c = exp(-dG);
            q_f = k_f * cs[1] * cs[50];
            q_b = -k_f / K_c * cs[0] * cs[49];
            q[313] = q_f + q_b;

            // Reaction #314
            k_f = exp(log(31600.000000000004) + 1.8 * logT -
                      (470.0070442878472 * Tinv));
            dG = -gbs[4] + gbs[5] + gbs[49] - gbs[50];
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[50];
            q_b = -k_f / K_c * cs[5] * cs[49];
            q[314] = q_f + q_b;

            // Reaction #315
            k_f = exp(log(0.37800000000000006) + 2.72 * logT -
                      (754.8293002481486 * Tinv));
            dG = gbs[6] - gbs[7] - gbs[49] + gbs[50];
            K_c = exp(-dG);
            q_f = k_f * cs[7] * cs[49];
            q_b = -k_f / K_c * cs[6] * cs[50];
            q[315] = q_f + q_b;

            // Reaction #316
            k_f = exp(log(0.0009030000000000002) + 3.65 * logT -
                      (3600.03254265017 * Tinv));
            dG = -gbs[12] + gbs[13] + gbs[49] - gbs[50];
            K_c = exp(-dG);
            q_f = k_f * cs[12] * cs[50];
            q_b = -k_f / K_c * cs[13] * cs[49];
            q[316] = q_f + q_b;

            // Reaction #317
            k_f = exp(log(2550.0000000000005) + 1.6 * logT -
            ...
            // Reaction #320
            k_f = exp(log(4060.0000000000005) + 2.19 * logT -
                      (447.8653848139015 * Tinv));
            ...
            K_c = exp(-dG);
            q_f = k_f * cs[4] * cs[49];
            q_b = -k_f / K_c * cs[18] * cs[25];
            q[321] = q_f + q_b;

            // Reaction #322
            k_f = exp(log(25500000.000000004) + 0.255 * logT -
                      (-474.5360200893361 * Tinv));
            dG = gbs[3] - gbs[6] - gbs[49] + gbs[50];
            K_c = exp(-dG);
            q_f = k_f * cs[6] * cs[49];
            q_b = -k_f / K_c * cs[3] * cs[50];
            q[322] = q_f + q_b;
          ...
          dYdt[0] =
              th.MW(0) *
              (-q[2] + q[7] + q[38] + q[39] + q[40] + q[41] + q[44] + q[46] +
               q[48] + q[50] + q[52] + q[54] + q[57] + q[59] + q[64] + q[67] +
               q[68] + q[72] + q[74] + q[76] + q[77] + q[79] - q[82] - q[83] -
               q[125] - q[135] + q[136] - q[145] - q[171] + q[173] + q[190] +
               q[196] + q[201] + q[208] + q[213] - q[220] + q[265] + q[275] +
               q[276] + q[283] + q[287] - q[288] + q[292] + q[298] + q[299] +
               q[308] + q[313]);
          dYdt[1] =
              th.MW(1) *
              (-q[1] + q[2] + q[5] + q[6] + q[8] + q[9] + q[13] + q[20] +
               q[23] + q[27] - q[32] - q[33] - q[34] - q[35] - q[36] - q[37] -
               2.0 * q[38] - 2.0 * q[39] - 2.0 * q[40] - 2.0 * q[41] - q[42] -
               q[43] - q[44] - q[45] - q[46] - q[47] - q[48] - q[49] - q[50] -
               q[51] - q[52] - q[53] - q[54] - q[55] - q[56] - q[57] - q[58] -
               q[59] - q[60] - q[61] - q[62] - q[64] - q[65] - q[66] - q[67] -
               q[68] - q[69] - q[70] - q[71] - q[72] - q[73] - q[74] - q[75] -
               q[76] - q[77] - q[78] - q[79] - q[80] + q[83] + q[89] + q[90] +
               q[91] + q[93] + q[98] + q[105] + q[106] + q[107] + q[122] +
               q[123] + q[125] + q[126] + q[127] + q[128] + q[129] + q[132] +
               q[134] + q[135] + q[137] + q[143] + q[145] + q[148] + q[158] +
               q[165] + q[166] + q[171] + q[179] - q[182] - q[188] + q[189] -
               q[190] + q[191] + q[195] + q[198] + q[200] - q[201] + q[203] +
               q[204] - q[208] - q[211] - q[213] + q[217] + q[220] - q[222] +
               q[223] + q[229] + q[230] + q[233] + q[234] - q[236] + q[246] +
               q[248] + q[250] + q[251] + q[253] + q[256] + q[259] - q[260] -
               q[264] - q[265] - q[270] - q[271] + q[274] - q[276] + q[283] +
               q[284] + q[285] + 2.0 * q[289] + 2.0 * q[291] - q[298] - q[299] -
               q[303] + q[304] - q[307] - q[308] - q[313] - q[319] - q[320]);
          dYdt[2] =
              th.MW(2) *
              (-2.0 * q[0] - q[1] - q[2] - q[3] - q[4] - q[5] - q[6] - q[7] -
               q[8] - q[9] - q[10] - q[11] - q[12] - q[13] - q[14] - q[15] -
               q[16] - q[17] - q[18] - q[19] - q[20] - q[21] - q[22] - q[23] -
               q[24] - q[25] - q[26] - q[27] - q[28] - q[29] + q[30] + q[37] +
               q[43] + q[85] + q[121] + q[124] + q[154] + q[177] + q[178] -
               q[180] - q[181] + q[184] - q[186] - q[187] - q[189] + q[193] -
               q[199] - q[200] - q[206] - q[207] - q[212] - q[216] + q[219] -
               q[221] - q[230] - q[231] - q[232] + q[243] + q[245] - q[256] -
               q[257] + q[258] - q[261] - q[262] - q[263] - q[278] - q[283] -
               q[284] - q[285] + q[290] + q[293] - q[295] - q[296] - q[304] -
               q[312] - q[318]);
          dYdt[3] =
              th.MW(3) *
              (q[0] + q[3] - q[30] - q[31] - q[32] - q[33] - q[34] - q[35] -
               q[36] - q[37] + q[44] + q[86] + q[114] + q[115] + q[117] -
               q[121] - q[124] - q[134] - q[143] - q[144] - q[154] - q[155] -
               q[167] - q[168] - q[169] - q[170] - q[172] - q[174] - q[175] -
               q[178] + q[180] + q[187] - q[193] - q[194] - q[205] - q[215] -
               q[219] - q[225] - q[258] + q[286] - q[289] - q[290] - q[293] -
               q[294] - q[297] - q[305] - q[306] + q[322]);
          dYdt[4] =
              th.MW(4) *
              (q[1] + q[2] + q[3] + q[4] + q[10] + q[12] + q[14] + q[15] +
               q[16] + q[17] + q[18] + q[21] + q[26] + q[28] + q[37] - q[42] +
               2.0 * q[45] + q[47] + q[60] + q[65] - q[83] - 2.0 * q[84] -
               2.0 * q[85] - q[86] - q[87] - q[88] - q[89] - q[90] - q[91] -
               q[92] - q[93] - q[94] - q[95] - q[96] - q[97] - q[98] - q[99] -
               q[100] - q[101] - q[102] - q[103] - q[104] - q[105] - q[106] -
               q[107] - q[108] - q[109] - q[110] - q[111] - q[112] - q[113] +
               q[116] + q[118] + q[119] + q[134] + q[143] + q[155] + q[175] -
               q[179] + q[182] - q[183] + q[185] + q[188] - q[191] - q[192] +
               q[194] + q[197] + q[199] - q[202] + q[206] - q[209] + q[212] -
               q[214] - q[217] + q[218] - q[223] + q[232] - q[233] - q[234] -
               q[235] + q[249] + q[252] + q[255] - q[259] + q[263] - q[266] -
               q[267] + q[270] - q[277] + q[278] - q[286] - q[287] + q[295] +
               q[296] - q[300] + q[305] + q[306] - q[309] - q[310] + q[312] -
               q[314] - q[321] + q[323]);
          dYdt[5] =
              th.MW(5) *
              (q[42] + q[43] + q[47] + q[61] + q[66] + q[83] + q[85] + q[86] +
               q[87] + q[88] + q[92] + q[95] + q[96] + q[97] + q[99] + q[100] +
               q[101] + q[102] + q[103] + q[104] + q[108] + q[110] + q[111] +
               q[112] + q[113] - q[126] + q[144] - q[146] + q[192] - q[196] +
               q[202] + q[209] + q[214] - q[218] + q[254] + q[266] + q[277] +
               q[286] - q[292] + q[300] + q[309] + q[314]);
          dYdt[6] =
              th.MW(6) *
              (-q[3] + q[4] + q[31] + q[32] + q[33] + q[34] + q[35] + q[36] -
               q[43] - q[44] - q[45] + q[46] - q[86] + q[87] + q[88] -
               2.0 * q[114] - 2.0 * q[115] - q[116] - q[117] - q[118] - q[119] -
               q[120] + q[156] + q[167] + q[168] + q[169] + q[174] + q[183] -
               q[185] + q[205] + q[215] - q[286] + q[294] + q[297] - q[301] +
               q[315] - q[322] - q[323]);
          dYdt[7] =
              th.MW(7) * (-q[4] - q[46] - q[47] + q[84] - q[87] - q[88] +
                          q[114] + q[115] + q[120] - q[156] + q[301] - q[315]);
          dYdt[8] = th.MW(8) * (q[48] - q[89] - q[121] - q[122] - q[123] -
                                q[238] - q[243] - q[244]);
          dYdt[9] =
              th.MW(9) *
              (-q[5] + q[19] - q[48] + q[50] - q[90] + q[92] - q[124] - q[125] -
               q[126] - q[127] - q[128] - q[129] - q[130] - q[131] - q[132] -
               q[133] - q[239] - q[240] - q[245] - q[246] - q[247] - q[288]);
          dYdt[10] =
              th.MW(10) *
              (-q[6] + q[22] + q[29] - q[49] - q[91] - q[92] + q[95] - q[116] -
               q[122] + q[125] - q[127] - q[134] - q[135] - 2.0 * q[136] -
               q[137] - q[138] - q[139] - q[140] + q[141] + q[142] + q[147] +
               q[150] + q[151] + q[237] - q[241] - q[248] - q[249] - q[250] +
               q[260] - q[289] - q[290] - 2.0 * q[291] + q[304]);
          dYdt[11] = th.MW(11) * (-q[7] - q[8] - q[50] + q[61] + q[66] + q[78] -
                                  q[93] + q[96] - q[141] - q[142] - q[143] -
                                  q[144] - q[145] - q[146] - q[147] - q[148] -
                                  q[149] - q[150] - q[151] - q[152] - q[153] -
                                  q[242] - q[251] - q[252] - q[253] - q[292]);
          dYdt[12] =
              th.MW(12) *
              (-q[9] + q[10] + q[24] + q[25] + q[49] - q[51] + q[52] + q[60] +
               q[65] + q[80] - q[94] - q[95] - q[96] + q[97] + q[109] - q[117] -
               q[118] - q[123] - q[128] + q[135] - q[137] + 2.0 * q[138] +
               q[145] - q[148] + 2.0 * q[149] + q[153] - q[154] - q[155] -
               q[156] - 2.0 * q[157] - 2.0 * q[158] - q[159] - q[160] - q[161] -
               q[162] - q[163] - q[164] - q[210] - q[254] - q[255] - q[274] -
               q[275] - q[283] - q[287] + q[288] + q[296] + q[297] + q[299] +
               q[300] + q[301] + q[307] - q[311] - q[316] - q[317] + q[320] -
               q[324]);
          dYdt[13] =
              th.MW(13) * (-q[10] + q[51] - q[52] - q[97] + q[117] - q[129] -
                           q[138] - q[149] + q[156] + q[159] + q[160] + q[161] +
                           q[162] + q[163] + q[164] + q[210] + q[302] + q[316]);
          dYdt[14] =
              th.MW(14) *
              (q[5] + q[7] - q[11] + q[12] + q[19] + q[22] + 2.0 * q[27] -
               q[30] + q[54] + q[78] + q[80] - q[82] + q[89] - q[98] + q[99] +
               q[109] - q[119] + q[121] - q[130] + q[131] + q[133] + q[134] -
               q[139] + q[140] + q[143] + q[144] + q[152] + q[159] + q[165] +
               q[166] + q[167] + q[170] + 2.0 * q[175] + 2.0 * q[176] + q[216] +
               q[221] + q[222] + q[223] + q[224] + q[226] + q[227] + q[231] +
               q[235] + q[244] + q[256] + q[262] + q[264] + q[268] + q[271] +
               q[273] + q[279] + q[282] + q[283] + q[296] + q[297] + q[299] +
               q[300] + q[301] + q[302] + q[305]);
          dYdt[15] =
              th.MW(15) * (q[11] + q[13] + q[29] + q[30] + q[98] + q[119] -
                           q[131] - q[152] + q[225] + q[228] + q[261] + q[267] -
                           q[279] + q[281] - q[282] + q[289] + q[304]);
          dYdt[16] =
              th.MW(16) * (q[6] + q[8] - q[12] - q[13] + q[14] + q[24] + q[31] -
                           q[53] - q[54] + q[57] + q[90] - q[99] + q[100] +
                           q[120] + q[124] + q[131] - q[159] + q[160] - q[165] -
                           q[166] - q[167] + q[170] + q[172] + q[247] + q[258] +
                           q[259] + 2.0 * q[306] + q[307] + q[310]);
          dYdt[17] =
              th.MW(17) *
              (q[9] - q[14] + q[15] + q[16] + q[25] - q[31] + q[53] - q[55] -
               q[56] - q[57] + q[59] + q[64] + q[82] + q[91] + q[93] - q[100] +
               q[101] + q[102] + q[116] - q[120] + q[126] - q[132] + q[152] +
               q[155] - q[160] + q[168] + q[169] + q[172] + q[287] + q[290] +
               q[292] + q[305] + q[318] + q[323]);
          dYdt[18] = th.MW(18) * (-q[15] + q[17] + q[55] - q[58] - q[59] -
                                  q[60] - q[61] + q[63] + q[67] - q[101] +
                                  q[103] + q[161] - q[168] + q[310] + q[321]);
          dYdt[19] = th.MW(19) * (-q[16] + q[18] + q[56] - q[62] - q[63] -
                                  q[64] - q[65] - q[66] + q[68] - q[102] +
                                  q[104] + q[118] + q[154] + q[162] - q[169]);
          dYdt[20] =
              th.MW(20) * (-q[17] - q[18] + q[58] + q[62] - q[67] - q[68] +
                           q[94] - q[103] - q[104] + q[146] - q[161] - q[162]);
          dYdt[21] = th.MW(21) * (-q[19] + q[21] - q[69] - q[105] + q[108] +
                                  q[122] - q[170] - q[171]);
          dYdt[22] = th.MW(22) * (-q[20] - q[21] - q[22] + q[69] - q[70] +
                                  q[72] - q[106] - q[107] - q[108] - q[109] +
                                  q[110] + q[123] + q[127] + q[133] + q[136] +
                                  q[171] + q[173] + q[176] + q[291] + q[294]);
          dYdt[23] = th.MW(23) *
                     (-q[23] + q[70] - q[71] - q[72] + q[74] - q[110] + q[111] +
                      q[128] + q[140] + q[163] - q[172] - q[293] - q[294]);
          dYdt[24] = th.MW(24) * (-q[24] + q[71] - q[73] - q[74] + q[76] -
                                  q[111] + q[129] + q[137] + q[148] - q[163] -
                                  q[173] + q[174] - q[284] - q[317]);
          dYdt[25] = th.MW(25) *
                     (-q[25] + q[26] + q[73] - q[75] - q[76] + q[77] + q[112] +
                      q[153] + q[158] + q[164] - q[174] - q[285] - q[311] +
                      q[318] + q[320] + q[321] + q[323] + 2.0 * q[324]);
          dYdt[26] = th.MW(26) * (-q[26] + q[75] - q[77] - q[112] - q[153] +
                                  q[157] - q[164]);
          dYdt[27] = th.MW(27) * (q[20] - q[27] + q[28] - q[78] + q[79] +
                                  q[105] + q[113] + q[130] - q[133] - q[140] -
                                  q[175] - 2.0 * q[176] - q[273]);
          dYdt[28] = th.MW(28) *
                     (q[23] - q[28] - q[29] - q[79] - q[80] + q[81] + q[106] -
                      q[113] + q[132] + q[139] - q[303] + q[308] + q[309]);
          dYdt[29] = th.MW(29) * (-q[81] + q[107]);
          dYdt[30] =
              th.MW(30) * (-q[177] - q[178] - q[179] + q[190] + q[192] -
                           q[195] + q[216] - q[224] + q[226] - q[237] + q[238] +
                           q[239] + q[244] + q[247] - q[274] - q[275] - q[282]);
          dYdt[31] =
              th.MW(31) *
              (-q[189] - q[190] - q[191] - q[192] - q[193] - q[194] - q[195] -
               q[196] - q[197] - q[198] + q[199] + q[201] + q[202] + q[207] +
               q[222] + q[231] + q[241] + q[242] + q[261] + q[268] - q[279]);
          dYdt[32] =
              th.MW(32) * (-q[199] - q[200] - q[201] - q[202] + q[235] +
                           q[264] + q[267] + q[271] + q[276] + q[277] + q[278]);
          dYdt[33] = th.MW(33) * (-q[276] - q[277] - q[278]);
          dYdt[34] = th.MW(34) * (-q[203] - q[204] - q[205] - q[206] - q[207] -
                                  q[208] - q[209] - q[210]);
          dYdt[35] =
              th.MW(35) *
              (-q[177] + q[178] + q[179] + 2.0 * q[181] - q[185] - q[186] +
               q[187] + q[188] + q[189] + q[194] - q[197] - q[198] + q[207] -
               q[211] + q[212] + q[213] + q[214] + q[215] + q[221] + q[223] +
               q[225] - q[227] - q[228] - q[243] - q[244] - q[245] - q[246] -
               q[247] - q[248] - q[249] - q[250] - q[251] - q[252] - q[253] -
               q[254] - q[255] + q[257] - q[273] + q[280] + q[282]);
          dYdt[36] =
              th.MW(36) * (q[185] + q[186] - q[187] - q[188] - q[280] - q[281]);
          dYdt[37] = th.MW(37) * (-q[180] - q[181] - q[182] - q[183] - q[184] +
                                  q[198] + q[227] + q[281]);
          dYdt[38] =
              th.MW(38) * (q[191] + q[193] + q[196] + q[200] + q[211] - q[212] -
                           q[213] - q[214] - q[215] + q[262] + q[279]);
          dYdt[39] = th.MW(39) * (-q[216] - q[217] - q[218] - q[219] - q[220] +
                                  q[229] + q[232] + q[238] + q[243] - q[280]);
          dYdt[40] = th.MW(40) * (q[218] + q[220] - q[229] - q[230] - q[231] -
                                  q[232] - q[233] - q[234] - q[235] - q[236] +
                                  q[239] + q[241] + q[242] + q[245] + q[249] +
                                  q[252] + q[254] + q[257] + q[270] + q[275]);
          dYdt[41] = th.MW(41) * (q[236] - q[237] + q[255] + q[274]);
          dYdt[42] =
              th.MW(42) * (q[240] - q[256] - q[257] - q[258] - q[259] - q[260]);
          dYdt[43] =
              th.MW(43) * (q[250] + q[253] - q[269] - q[270] - q[271] + q[273]);
          dYdt[44] = th.MW(44) * (q[233] - q[272]);
          dYdt[45] = th.MW(45) * (q[234] + q[248] + q[251] - q[261] - q[262] -
                                  q[263] - q[264] - q[265] - q[266] - q[267] -
                                  q[268] + q[269] + q[272]);
          dYdt[46] =
              th.MW(46) * (q[217] + q[219] - q[221] - q[222] - q[223] - q[224] -
                           q[225] - q[226] - q[227] - q[228] + q[230] + q[246] +
                           q[263] + q[265] + q[266] + q[280] - q[281]);
          dYdt[47] =
              th.MW(47) *
              (q[177] + q[180] + q[182] + q[183] + q[184] + q[195] + q[197] +
               q[203] + q[204] + q[205] + q[206] + q[208] + q[209] + q[210] +
               q[224] + q[228] + q[237] - q[238] - q[239] - q[240] - q[241] -
               q[242] + q[256] + q[258] + q[259] + q[260]);
          dYdt[48] = th.MW(48) * (0.0);
          dYdt[49] = th.MW(49) * (q[312] + q[313] + q[314] - q[315] + q[316] +
                                  q[317] - q[318] - q[319] - q[320] - q[321] -
                                  q[322] - q[323] - q[324]);
          dYdt[50] = th.MW(50) * (q[311] - q[312] - q[313] - q[314] + q[315] -
                                  q[316] + q[319] + q[322]);
          dYdt[51] =
              th.MW(51) * (q[284] + q[293] + q[295] + q[298] + q[303] - q[304] -
                           q[305] - q[306] - q[307] - q[308] - q[309] - q[310]);
          dYdt[52] = th.MW(52) * (q[285] - q[295] - q[296] - q[297] - q[298] -
