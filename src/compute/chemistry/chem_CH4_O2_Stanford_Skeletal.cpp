...
// --------------------------------------------------------------|
  ...
  double q_f, q_b;
  double q[38];
  q_f =   S_tbc[0] * k_f[0] * cs[1] * cs[2];
  q_b = - S_tbc[0] * k_f[0]/K_c[0] * cs[3] * cs[4];
  q[0] =   q_f + q_b;

  q_f =   S_tbc[1] * k_f[1] * cs[0] * cs[3];
  q_b = - S_tbc[1] * k_f[1]/K_c[1] * cs[1] * cs[4];
  q[1] =   q_f + q_b;

  q_f =   S_tbc[2] * k_f[2] * cs[0] * cs[3];
  q_b = - S_tbc[2] * k_f[2]/K_c[2] * cs[1] * cs[4];
  q[2] =   q_f + q_b;

  q_f =   S_tbc[3] * k_f[3] * cs[0] * cs[4];
  q_b = - S_tbc[3] * k_f[3]/K_c[3] * cs[1] * cs[6];
  q[3] =   q_f + q_b;

  q_f =   S_tbc[4] * k_f[4] * pow(cs[4],2.0);
  q_b = - S_tbc[4] * k_f[4]/K_c[4] * cs[3] * cs[6];
  q[4] =   q_f + q_b;

  q_f =   S_tbc[5] * k_f[5] * cs[0];
  q_b = - S_tbc[5] * k_f[5]/K_c[5] * pow(cs[1],2.0);
  q[5] =   q_f + q_b;

  q_f =   S_tbc[6] * k_f[6] * cs[1] * cs[3];
  q_b = - S_tbc[6] * k_f[6]/K_c[6] * cs[4];
  q[6] =   q_f + q_b;

  q_f =   S_tbc[7] * k_f[7] * cs[6];
  q_b = - S_tbc[7] * k_f[7]/K_c[7] * cs[1] * cs[4];
  q[7] =   q_f + q_b;

  q_f =   S_tbc[8] * k_f[8] * pow(cs[6],2.0);
  q_b = - S_tbc[8] * k_f[8]/K_c[8] * cs[1] * cs[4] * cs[6];
  q[8] =   q_f + q_b;

  q_f =   k_f[9] * cs[1] * cs[2];
  q_b = - k_f[9]/K_c[9] * cs[5];
  q[9] =   q_f + q_b;

  q_f =   S_tbc[10] * k_f[10] * cs[1] * cs[5];
  q_b = - S_tbc[10] * k_f[10]/K_c[10] * cs[0] * cs[2];
  q[10] =   q_f + q_b;

  q_f =   S_tbc[11] * k_f[11] * cs[1] * cs[5];
  q_b = - S_tbc[11] * k_f[11]/K_c[11] * pow(cs[4],2.0);
  q[11] =   q_f + q_b;

  q_f =   S_tbc[12] * k_f[12] * cs[1] * cs[5];
  q_b = - S_tbc[12] * k_f[12]/K_c[12] * cs[3] * cs[6];
  q[12] =   q_f + q_b;

  q_f =   S_tbc[13] * k_f[13] * cs[3] * cs[5];
  q_b = - S_tbc[13] * k_f[13]/K_c[13] * cs[2] * cs[4];
  q[13] =   q_f + q_b;

  q_f =   S_tbc[14] * k_f[14] * cs[4] * cs[5];
  q_b = - S_tbc[14] * k_f[14]/K_c[14] * cs[2] * cs[6];
  q[14] =   q_f + q_b;

  q_f =   S_tbc[15] * k_f[15] * cs[4] * cs[5];
  q_b = - S_tbc[15] * k_f[15]/K_c[15] * cs[2] * cs[6];
  q[15] =   q_f + q_b;

  q_f =   k_f[16] * cs[3] * cs[9];
  q_b = - k_f[16]/K_c[16] * cs[11];
  q[16] =   q_f + q_b;

  q_f =   S_tbc[17] * k_f[17] * cs[2] * cs[9];
  q_b = - S_tbc[17] * k_f[17]/K_c[17] * cs[3] * cs[11];
  q[17] =   q_f + q_b;

  q_f =   S_tbc[18] * k_f[18] * cs[4] * cs[9];
  q_b = - S_tbc[18] * k_f[18]/K_c[18] * cs[1] * cs[11];
  q[18] =   q_f + q_b;

  q_f =   S_tbc[19] * k_f[19] * cs[4] * cs[9];
  q_b = - S_tbc[19] * k_f[19]/K_c[19] * cs[1] * cs[11];
  q[19] =   q_f + q_b;

  q_f =   S_tbc[20] * k_f[20] * cs[5] * cs[9];
  q_b = - S_tbc[20] * k_f[20]/K_c[20] * cs[4] * cs[11];
  q[20] =   q_f + q_b;

  q_f =   S_tbc[21] * k_f[21] * cs[1] * cs[8];
  q_b = - S_tbc[21] * k_f[21]/K_c[21] * cs[0] * cs[7];
  q[21] =   q_f + q_b;

  q_f =   S_tbc[22] * k_f[22] * cs[3] * cs[8];
  q_b = - S_tbc[22] * k_f[22]/K_c[22] * cs[4] * cs[7];
  q[22] =   q_f + q_b;

  q_f =   S_tbc[23] * k_f[23] * cs[4] * cs[8];
  q_b = - S_tbc[23] * k_f[23]/K_c[23] * cs[6] * cs[7];
  q[23] =   q_f + q_b;

  q_f =   k_f[24] * cs[1] * cs[7];
  q_b = - k_f[24]/K_c[24] * cs[8];
  q[24] =   q_f + q_b;

  q_f =   S_tbc[25] * k_f[25] * cs[3] * cs[7];
  q_b = - S_tbc[25] * k_f[25]/K_c[25] * cs[1] * cs[10];
  q[25] =   q_f + q_b;

  q_f =   S_tbc[26] * k_f[26] * cs[3] * cs[7];
  q_b = - S_tbc[26] * k_f[26]/K_c[26] * cs[0] * cs[1] * cs[9];
  q[26] =   q_f;

  q_f =   S_tbc[27] * k_f[27] * cs[5] * cs[7];
  q_b = - S_tbc[27] * k_f[27]/K_c[27] * cs[2] * cs[8];
  q[27] =   q_f + q_b;

  q_f =   S_tbc[28] * k_f[28] * cs[5] * cs[7];
  q_b = - S_tbc[28] * k_f[28]/K_c[28] * cs[1] * cs[4] * cs[10];
  q[28] =   q_f;

  q_f =   S_tbc[29] * k_f[29] * cs[2] * cs[7];
  q_b = - S_tbc[29] * k_f[29]/K_c[29] * cs[1] * cs[3] * cs[10];
  q[29] =   q_f;

  q_f =   S_tbc[30] * k_f[30] * cs[2] * cs[7];
  q_b = - S_tbc[30] * k_f[30]/K_c[30] * cs[4] * cs[10];
  q[30] =   q_f + q_b;

  q_f =   S_tbc[31] * k_f[31] * cs[7] * cs[10];
  q_b = - S_tbc[31] * k_f[31]/K_c[31] * cs[1] * cs[8] * cs[9];
  q[31] =   q_f;

  q_f =   k_f[32] * cs[10];
  q_b = - k_f[32]/K_c[32] * cs[0] * cs[9];
  q[32] =   q_f + q_b;

  q_f =   S_tbc[33] * k_f[33] * cs[1] * cs[10];
  q_b = - S_tbc[33] * k_f[33]/K_c[33] * cs[0] * cs[1] * cs[9];
  q[33] =   q_f;

  q_f =   S_tbc[34] * k_f[34] * cs[1] * cs[10];
  q_b = - S_tbc[34] * k_f[34]/K_c[34] * cs[0] * cs[1] * cs[9];
  q[34] =   q_f;

  q_f =   S_tbc[35] * k_f[35] * cs[3] * cs[10];
  q_b = - S_tbc[35] * k_f[35]/K_c[35] * cs[1] * cs[4] * cs[9];
  q[35] =   q_f;

  q_f =   S_tbc[36] * k_f[36] * cs[4] * cs[10];
  q_b = - S_tbc[36] * k_f[36]/K_c[36] * cs[1] * cs[6] * cs[9];
  q[36] =   q_f;

  q_f =   S_tbc[37] * k_f[37] * cs[2] * cs[10];
  q_b = - S_tbc[37] * k_f[37]/K_c[37] * cs[1] * cs[5] * cs[9];
  q[37] =   q_f;
