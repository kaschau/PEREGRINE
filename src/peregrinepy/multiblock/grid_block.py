...
class grid_block(topology_block):
    ...
    def init_grid_arrays(self):
        ...
        shape = [self.ni+2,self.nj+2,self.nk+2] if self.block_type == 'solver' else [self.ni,self.nj,self.nk]
        for name in ['x','y','z']:
            self.array[name] = np.zeros((shape))

        # Cell center locations, volumes, diffusive metrics
        shape = [self.ni+1,self.nj+1,self.nk+1] if self.block_type == 'solver' else [self.ni-1,self.nj-1,self.nk-1]
        ...
        shape = [self.ni+2,self.nj+1,self.nk+1] if self.block_type == 'solver' else [self.ni,self.nj-1,self.nk-1]
        for name in ['isx','isy','isz','iS','inx','iny','inz']:
            self.array[name] = np.zeros((shape))

        # j face normal, area vectors
        shape = [self.ni+1,self.nj+2,self.nk+1] if self.block_type == 'solver' else [self.ni-1,self.nj,self.nk-1]
        for name in ['jsx','jsy','jsz','jS','jnx','jny','jnz']:
            self.array[name] = np.zeros((shape))

        # k face normal, area vectors
        shape = [self.ni+1,self.nj+1,self.nk+2] if self.block_type == 'solver' else [self.ni-1,self.nj-1,self.nk]
