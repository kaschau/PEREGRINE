    ...
    for var in varis:
        ...
        for blk in mb:
            ...
            for face in blk.faces:
                ...
                ssize = recv.size
                reqs.append(
                    comm.Irecv([recv, ssize, MPIDOUBLE], source=comm_rank, tag=tag)
                )
        ...
        for blk in mb:
            ...
            for face in blk.faces:
                ...
                send, slice_s = (
                    (face.sendbuffer4, face.slice_s4)
                    if ndim == 4
                    else (face.sendbuffer3, face.slice_s3)
                )
        ...
        for blk in mb:
            ...
            for face in blk.faces:
                ...
                recv, slice_r = (
                    (face.recvbuffer4, face.slice_r4)
                    if ndim == 4
                    else (face.recvbuffer3, face.slice_r3)
                )
        ...
        for face in blk.faces:
            ...
            if neighbor is None:
                ...
                face_orientations = [
                    ...
                    if j != face_to_orient_place_mapping[face.nface]
                ]
                normal_index = [
                    j for j in range(3) if j == face_to_orient_place_mapping[face.nface]
                ][0]
                face_orientations2 = [
                    int(i)
                    for j, i in enumerate(orientation2)
                    if j != face_to_orient_place_mapping[face2]
                ]
                normal_index2 = [
                    j for j in range(3) if j == face_to_orient_place_mapping[face2]
                ][0]
