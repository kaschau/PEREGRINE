...
def metrics(blk, fdOrder=2):
    ...
    if fdOrder == 2:
        ...
        dxdE = (
            -xc[4::, 2:-2, 2:-2] + 8.0 * xc[3:-1, 2:-2, 2:-2] - 8.0 * xc[1:-3, 2:-2, 2:-2] + xc[0:-4, 2:-2, 2:-2]
        ) / 12.0
        dxdN = (
            -xc[2:-2, 4::, 2:-2] + 8.0 * xc[2:-2, 3:-1, 2:-2] - 8.0 * xc[2:-2, 1:-3, 2:-2] + xc[2:-2, 0:-4, 2:-2]
        ) / 12.0
        dxdX = (
            -xc[2:-2, 2:-2, 4::] + 8.0 * xc[2:-2, 2:-2, 3:-1] - 8.0 * xc[2:-2, 2:-2, 1:-3] + xc[2:-2, 2:-2, 0:-4]
        ) / 12.0

        dydE = (
            -yc[4::, 2:-2, 2:-2] + 8.0 * yc[3:-1, 2:-2, 2:-2] - 8.0 * yc[1:-3, 2:-2, 2:-2] + yc[0:-4, 2:-2, 2:-2]
        ) / 12.0
        dydN = (
            -yc[2:-2, 4::, 2:-2] + 8.0 * yc[2:-2, 3:-1, 2:-2] - 8.0 * yc[2:-2, 1:-3, 2:-2] + yc[2:-2, 0:-4, 2:-2]
        ) / 12.0
        dydX = (
            -yc[2:-2, 2:-2, 4::] + 8.0 * yc[2:-2, 2:-2, 3:-1] - 8.0 * yc[2:-2, 2:-2, 1:-3] + yc[2:-2, 2:-2, 0:-4]
        ) / 12.0

        dzdE = (
            -zc[4::, 2:-2, 2:-2] + 8.0 * zc[3:-1, 2:-2, 2:-2] - 8.0 * zc[1:-3, 2:-2, 2:-2] + zc[0:-4, 2:-2, 2:-2]
        ) / 12.0
        dzdN = (
            -zc[2:-2, 4::, 2:-2] + 8.0 * zc[2:-2, 3:-1, 2:-2] - 8.0 * zc[2:-2, 1:-3, 2:-2] + zc[2:-2, 0:-4, 2:-2]
        ) / 12.0
        dzdX = (
            -zc[2:-2, 2:-2, 4::] + 8.0 * zc[2:-2, 2:-2, 3:-1] - 8.0 * zc[2:-2, 2:-2, 1:-3] + zc[2:-2, 2:-2, 0:-4]
        ) / 12.0
