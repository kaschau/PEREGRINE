...
#----------------------------------------------------------------------------
    ...
    blk.array['xc'][:] = 0.125*( x[0:-1, 0:-1, 0:-1  ] + x[0:-1, 0:-1, 1::] +
                                 x[0:-1, 1:: , 0:-1  ] + x[0:-1, 1:: , 1::] +
                                 x[1:: , 0:-1, 0:-1  ] + x[1:: , 0:-1, 1::] +
                                 x[1:: , 1:: , 0:-1  ] + x[1:: , 1:: , 1::]  )

    blk.array['yc'][:] = 0.125*( y[0:-1, 0:-1, 0:-1  ] + y[0:-1, 0:-1, 1::] +
                                 y[0:-1, 1:: , 0:-1  ] + y[0:-1, 1:: , 1::] +
                                 y[1:: , 0:-1, 0:-1  ] + y[1:: , 0:-1, 1::] +
                                 y[1:: , 1:: , 0:-1  ] + y[1:: , 1:: , 1::] )

    blk.array['zc'][:] = 0.125*( z[0:-1, 0:-1, 0:-1  ] + z[0:-1, 0:-1, 1::] +
                                 z[0:-1, 1:: , 0:-1  ] + z[0:-1, 1:: , 1::] +
                                 z[1:: , 0:-1, 0:-1  ] + z[1:: , 0:-1, 1::] +
                                 z[1:: , 1:: , 0:-1  ] + z[1:: , 1:: , 1::] )

#----------------------------------------------------------------------------
# i face area, normal vectors
#----------------------------------------------------------------------------

    blk.array['isx'][:] = 0.5 * ( (y[ : ,1:: , 1::]-y[ : ,0:-1, 0:-1]) *
                                  (z[ : ,0:-1, 1::]-z[ : ,1:: , 0:-1]) -
                                  (z[ : ,1:: , 1::]-z[ : ,0:-1, 0:-1]) *
                                  (y[ : ,0:-1, 1::]-y[ : ,1:: , 0:-1]) );

    blk.array['isy'][:] = 0.5 * ( (z[ : ,1:: , 1::]-z[ : ,0:-1, 0:-1]) *
                                  (x[ : ,0:-1, 1::]-x[ : ,1:: , 0:-1]) -
                                  (x[ : ,1:: , 1::]-x[ : ,0:-1, 0:-1]) *
                                  (z[ : ,0:-1, 1::]-z[ : ,1:: , 0:-1]) );


    blk.array['isz'][:] = 0.5 * ( (x[ : ,1:: , 1::]-x[ : ,0:-1, 0:-1]) *
                                  (y[ : ,0:-1, 1::]-y[ : ,1:: , 0:-1]) -
                                  (y[ : ,1:: , 1::]-y[ : ,0:-1, 0:-1]) *
                                  (x[ : ,0:-1, 1::]-x[ : ,1:: , 0:-1]) );

    blk.array['iS'] [:]= np.sqrt( blk.array['isx']**2 +
                                  blk.array['isy']**2 +
                                  blk.array['isz']**2 )
...
#----------------------------------------------------------------------------
# j face area, normal vectors
#----------------------------------------------------------------------------

    blk.array['jsx'][:] = 0.5 *( (y[1::, : , 1:: ]-y[0:-1 , : ,0:-1]) *
                                 (z[1::, : , 0:-1]-z[0:-1 , : ,1:: ]) -
                                 (z[1::, : , 1:: ]-z[0:-1 , : ,0:-1]) *
                                 (y[1::, : , 0:-1]-y[0:-1 , : ,1:: ]) );

    blk.array['jsy'][:] = 0.5 *( (z[1::, : , 1:: ]-z[0:-1 , : ,0:-1]) *
                                 (x[1::, : , 0:-1]-x[0:-1 , : ,1:: ]) -
                                 (x[1::, : , 1:: ]-x[0:-1 , : ,0:-1]) *
                                 (z[1::, : , 0:-1]-z[0:-1 , : ,1:: ]) );

    blk.array['jsz'][:] = 0.5 *( (x[1::, : , 1:: ]-x[0:-1 , : ,0:-1]) *
                                 (y[1::, : , 0:-1]-y[0:-1 , : ,1:: ]) -
                                 (y[1::, : , 1:: ]-y[0:-1 , : ,0:-1]) *
                                 (x[1::, : , 0:-1]-x[0:-1 , : ,1:: ]) );

    blk.array['jS'] [:]= np.sqrt( blk.array['jsx']**2 +
                                  blk.array['jsy']**2 +
                                  blk.array['jsz']**2 )
...
#----------------------------------------------------------------------------
# k face area, normal vectors
#----------------------------------------------------------------------------

    blk.array['ksx'][:] = 0.5 *( (y[1:: , 1::, : ]-y[0:-1, 0:-1, : ]) *
                                 (z[0:-1, 1::, : ]-z[1:: , 0:-1, : ]) -
                                 (z[1:: , 1::, : ]-z[0:-1, 0:-1, : ]) *
                                 (y[0:-1, 1::, : ]-y[1:: , 0:-1, : ]) );

    blk.array['ksy'][:] = 0.5 *( (z[1:: , 1::, : ]-z[0:-1, 0:-1, : ]) *
                                 (x[0:-1, 1::, : ]-x[1:: , 0:-1, : ]) -
                                 (x[1:: , 1::, : ]-x[0:-1, 0:-1, : ]) *
                                 (z[0:-1, 1::, : ]-z[1:: , 0:-1, : ]) );

    blk.array['ksz'][:] = 0.5 *( (x[1:: , 1::, : ]-x[0:-1, 0:-1, : ]) *
                                 (y[0:-1, 1::, : ]-y[1:: , 0:-1, : ]) -
                                 (y[1:: , 1::, : ]-y[0:-1, 0:-1, : ]) *
                                 (x[0:-1, 1::, : ]-x[1:: , 0:-1, : ]) );

    blk.array['kS'][:] = np.sqrt( blk.array['ksx']**2 +
                                  blk.array['ksy']**2 +
                                  blk.array['ksz']**2 )
...
#----------------------------------------------------------------------------
#
    blk.array['J'][:] = ( ( x[1::, 1::, 1::] - x[0:-1, 0:-1, 0:-1] )*( blk.array['isx'][1:: , :  , :  ]
                                                                     + blk.array['jsx'][ :  ,1:: , :  ]
                                                                     + blk.array['ksx'][ :  , :  ,1:: ] )
                        + ( y[1::, 1::, 1::] - y[0:-1, 0:-1, 0:-1] )*( blk.array['isy'][1:: , :  , :  ]
                                                                     + blk.array['jsy'][ :  ,1:: , :  ]
                                                                     + blk.array['ksy'][ :  , :  ,1:: ] )
                        + ( z[1::, 1::, 1::] - z[0:-1, 0:-1, 0:-1] )*( blk.array['isz'][1:: , :  , :  ]
                                                                     + blk.array['jsz'][ :  ,1:: , :  ]
                                                                     + blk.array['ksz'][ :  , :  ,1:: ] ) )/3.e0;
...
#cell corner
    x1 = x[0:-1,0:-1,0:-1]; x2 = x[0:-1,1::,0:-1]; x3 = x[1::,1::,0:-1]; x4 = x[1::,0:-1,0:-1]
    x5 = x[0:-1,0:-1,1:: ]; x6 = x[0:-1,1::,1:: ]; x7 = x[1::,1::,1:: ]; x8 = x[1::,0:-1,1:: ]

    y1 = y[0:-1,0:-1,0:-1]; y2 = y[0:-1,1::,0:-1]; y3 = y[1::,1::,0:-1]; y4 = y[1::,0:-1,0:-1]
    y5 = y[0:-1,0:-1,1:: ]; y6 = y[0:-1,1::,1:: ]; y7 = y[1::,1::,1:: ]; y8 = y[1::,0:-1,1:: ]

    z1 = z[0:-1,0:-1,0:-1]; z2 = z[0:-1,1::,0:-1]; z3 = z[1::,1::,0:-1]; z4 = z[1::,0:-1,0:-1]
    z5 = z[0:-1,0:-1,1:: ]; z6 = z[0:-1,1::,1:: ]; z7 = z[1::,1::,1:: ]; z8 = z[1::,0:-1,1:: ]
