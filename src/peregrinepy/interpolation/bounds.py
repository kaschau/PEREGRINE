...
def pts_in_blk_bounds(blk, test_pts):
    ...
    if ( xmin > np.max(test_pts[:,0]) or xmax < np.min(test_pts[:,0]) or
         ymin > np.max(test_pts[:,1]) or ymax < np.min(test_pts[:,1]) or
         zmin > np.max(test_pts[:,2]) or zmax < np.min(test_pts[:,2]) ):
        ...
        cube = np.array([[blk.array['x'][ 0, 0, 0], blk.array['y'][ 0, 0, 0], blk.array['z'][ 0, 0, 0]],
                         ...
                         [blk.array['x'][-1,-1, 0], blk.array['y'][-1,-1, 0], blk.array['z'][-1,-1, 0]],
                         [blk.array['x'][-1, 0,-1], blk.array['y'][-1, 0,-1], blk.array['z'][-1, 0,-1]],
                         [blk.array['x'][ 0,-1,-1], blk.array['y'][ 0,-1,-1], blk.array['z'][ 0,-1,-1]],
                         [blk.array['x'][-1,-1,-1], blk.array['y'][-1,-1,-1], blk.array['z'][-1,-1,-1]]])
        ...
        if blk.nx > 4:
            indx = int(blk.nx/2)
            cube = np.append(cube,
                             [[blk.array['x'][indx, 0, 0], blk.array['y'][indx, 0, 0], blk.array['z'][indx, 0, 0]],
                              [blk.array['x'][indx,-1, 0], blk.array['y'][indx,-1, 0], blk.array['z'][indx,-1, 0]],
                              [blk.array['x'][indx, 0,-1], blk.array['y'][indx, 0,-1], blk.array['z'][indx, 0,-1]],
                              [blk.array['x'][indx,-1,-1], blk.array['y'][indx,-1,-1], blk.array['z'][indx,-1,-1]]], axis=0)
            ...
            if blk.ny > 4:
                indx2 = int(blk.ny/2)
                cube = np.append(cube,
                                 [[blk.array['x'][indx,indx2, 0], blk.array['y'][indx,indx2, 0], blk.array['z'][indx,indx2, 0]],
                                  [blk.array['x'][indx,indx2,-1], blk.array['y'][indx,indx2,-1], blk.array['z'][indx,indx2,-1]]], axis=0)
            if blk.nz > 4:
                indx2 = int(blk.nz/2)
                cube = np.append(cube,
                                 [[blk.array['x'][indx, 0, indx2], blk.array['y'][indx, 0, indx2], blk.array['z'][ indx, 0, indx2]],
                                  [blk.array['x'][indx,-1, indx2], blk.array['y'][indx,-1, indx2], blk.array['z'][ indx,-1, indx2]]], axis=0)

        if blk.ny > 4:
            indx = int(blk.ny/2)
            cube = np.append(cube,
                             [[blk.array['x'][ 0, indx, 0], blk.array['y'][ 0, indx, 0], blk.array['z'][ 0, indx, 0]],
                              [blk.array['x'][-1, indx, 0], blk.array['y'][-1, indx, 0], blk.array['z'][-1, indx, 0]],
                              [blk.array['x'][ 0, indx,-1], blk.array['y'][ 0, indx,-1], blk.array['z'][ 0, indx,-1]],
                              [blk.array['x'][-1, indx,-1], blk.array['y'][-1, indx,-1], blk.array['z'][-1, indx,-1]]], axis=0)
            if blk.nz > 4:
                indx2 = int(blk.nz/2)
                cube = np.append(cube,
                                 [[blk.array['x'][indx, 0,indx2], blk.array['y'][indx, 0,indx2], blk.array['z'][indx, 0,indx2]],
        ...
        if blk.nz > 4:
            indx = int(blk.nz/2)
            cube = np.append(cube,
                             [[blk.array['x'][ 0, 0,indx], blk.array['y'][ 0, 0,indx], blk.array['z'][ 0, 0,indx]],
                              [blk.array['x'][-1, 0,indx], blk.array['y'][-1, 0,indx], blk.array['z'][-1, 0,indx]],
                              [blk.array['x'][ 0,-1,indx], blk.array['y'][ 0,-1,indx], blk.array['z'][ 0,-1,indx]],
                              [blk.array['x'][-1,-1,indx], blk.array['y'][-1,-1,indx], blk.array['z'][-1,-1,indx]]], axis=0)
...
def find_bounds(mb_to, mb_from, verbose_search):
    ...
    bounding_blocks = []
    for blk_to in mb_to:
        blk_to_xc = np.column_stack((blk_to.array['xc'].ravel(), blk_to.array['yc'].ravel(), blk_to.array['zc'].ravel()))
