...
def blocks_to_block(blks_from, blk_to, function='nearest', smooth=0.5):
    ...
    for i in range(5 + blk_to.ns - 1):
        qv_from = np.concatenate(tuple([blk.array['q'][1:-1,1:-1,1:-1,i].ravel() for blk in blks_from]))

        if function == 'nearest':
            interp_from = interpolate.NearestNDInterpolator(np.column_stack((blk_from_x, blk_from_y, blk_from_z)), qv_from)
        else:
            interp_from = interpolate.Rbf(blk_from_x, blk_from_y, blk_from_z, qv_from, function=function, smooth=smooth)

        qv_to = interp_from(blk_to.array['xc'].ravel(), blk_to.array['yc'].ravel(), blk_to.array['zc'].ravel())
        ...
        blk_to.qv[1:-1,1:-1,1:-1,i] = qv_to.reshape(blk_to.qv[1:-1,1:-1,1:-1,i].shape)
