...
def test_adiabaticSlipWall():
    ...
    for face in blk.faces:
        ...
        for s0_ in face.s0_:
            ...
            assert np.allclose(u[s0_], u[face.s1_] - 2.0 * u[face.s1_] * nx[face.s1_])
            assert np.allclose(v[s0_], v[face.s1_] - 2.0 * v[face.s1_] * ny[face.s1_])
            assert np.allclose(w[s0_], w[face.s1_] - 2.0 * w[face.s1_] * nz[face.s1_])
