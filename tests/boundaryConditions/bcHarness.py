...
class BaseBC:
    ...
    def run(self, face, stage):
        """Runs one hook on every face -- their halos are disjoint, so the
        face under test sees only its own -- and snapshots what it wrote."""
        self.mb.applyBcs(stage)
        for name in self._pull[stage]:
            self.host[name] = (
                primitives(self.mb, self.blk)
                if name == "q"
                else getattr(self.blk, name).get()
            )
    ...
    def normals(self, face):
        ...
        n = tuple(face.blockFacePlane(c) for c in normals)
        return n, (-1.0 if face.amILow else 1.0)

    # the planes either side of a face over the face proper: the halo's
    # edges and corners are never written, so never checked
    def halo(self, face, a):
        ng = self.blk.ng
        return [h[ng:-ng, ng:-ng] for h in face.halo(a)]

    def interior(self, face, a):
        ng = self.blk.ng
        return [h[ng:-ng, ng:-ng] for h in face.interior(a)]
    ...
    def mirror(self, face, name, where=None, sign=1.0):
        ...
        a = self.q(name)
        for h in self.halo(face, a):
            self._close(h, sign * self.interior(face, a)[0], where)
    ...
    def extrapolate(self, face, name, where=None, lo=None, hi=None):
        ...
        a = self.q(name)
        first = self.interior(face, a)[0]
        for h, deeper in zip(self.halo(face, a), self.interior(face, a)[1:]):
    ...
    def straddles(self, face, name, where=None):
        ...
        for h in self.halo(face, a):
            self._close(
                h, 2.0 * self._bcVals(face, name) - self.interior(face, a)[0], where
            )
    ...
    def alongNormal(self, face):
        ...
        velo = [self.q(c) for c in "uvw"]
        for g in range(self.blk.ng):
            Vb = sum(self.halo(face, c)[g] * ni for c, ni in zip(velo, n))
            for c, ni in zip(velo, n):
                self._close(self.halo(face, c)[g], Vb * ni)

    def _gradients(self, face, rules):
        """the first halo layer of every gradient, one rule per variable"""
        d = self.host["grads"]
        h, first = self.halo(face, d)[0], self.interior(face, d)[0]
