...
def test_231():
    ...
    for var,shape,off in zip(['x','y','z','q'],
        ...
        for k in range(shape[2]):
            for j in range(shape[1]):
                check0 = np.all(blk0.array[var][-3+off,j,k] == blk1.array[var][k,0,j])
                check1 = np.all(blk0.array[var][-1,j,k] == blk1.array[var][k,2-off,j])
                if not check0 or not check1:
                    break
            if not check0 or not check1:
                break
...
def test_321():
    ...
    for var,shape,off in zip(['x','y','z','q'],
        ...
        for k in range(shape[2]):
            for j in range(shape[1]):
                check0 = np.all(blk0.array[var][-3+off,j,k] == blk1.array[var][j,k,0])
                check1 = np.all(blk0.array[var][-1,j,k] == blk1.array[var][j,k,2-off])
                if not check0 or not check1:
...
def test_432():
    ...
    for var,shape,off in zip(['x','y','z','q'],
        ...
        for k in range(shape[2]):
            for j in range(shape[1]):
                check0 = np.all(blk0.array[var][-3+off,j,k] == blk1.array[var][-1,k,j])
                check1 = np.all(blk0.array[var][-1,j,k] == blk1.array[var][-3+off,k,j])
                if not check0 or not check1:
                    break
            if not check0 or not check1:
                break
...
def test_513():
    ...
    for var,shape,off in zip(['x','y','z','q'],
        ...
        for k in range(shape[2]):
            for j in range(shape[1]):
                check0 = np.all(blk0.array[var][-3+off,j,k] == blk1.array[var][j,-1,k])
                check1 = np.all(blk0.array[var][-1,j,k] == blk1.array[var][j,-3+off,k])
                if not check0 or not check1:
                    break
            if not check0 or not check1:
                break
...
def test_621():
    ...
    for var,shape,off in zip(['x','y','z','q'],
        ...
        for k in range(shape[2]):
            for j in range(shape[1]):
                check0 = np.all(blk0.array[var][-3+off,j,k] == blk1.array[var][k,j,-1])
                check1 = np.all(blk0.array[var][-1,j,k] == blk1.array[var][k,j,-3+off])
                if not check0 or not check1:
                    break
            if not check0 or not check1:
                break
