...
parser = argparse.ArgumentParser(description='Convert binary GridPro files into grid and connectivity files used by RAPTOR')
parser.add_argument('-blk', action='store', metavar='<GP_blk_FILE>', dest='gp_blk_file_name',
                    default='blk.tmp', help='Binary GridPro block node file. Default is blk.tmp', type=str)
parser.add_argument('-conn', action='store', metavar='<GP_conn_FILE>', dest='gp_conn_file_name',
                    default='blk.tmp.conn', help='GridPro block connectivity file. Default is blk.tmp.conn', type=str)
parser.add_argument('-pty', action='store', metavar='<GP_pty_FILE>', dest='gp_pty_file_name',
                    default='blk.tmp.pty', help='GridPro property file. Default is blk.tmp.pty', type=str)
parser.add_argument('--binary', action='store_true', dest='is_binary', help='Block file in binary format (default is ascii).')
...
gp_surface_to_raptor_bc = {'pdc:INTERBLK':'b 0',
                           'pdc:PERIODIC':'b 1',
                           'pdc:WALL':'s 1',
                           'pdc:user5':'i 1',
                           'pdc:user6':'e 1',
...
for blk in mb:
    ...
    if args.is_binary:
        ...
        temp = np.fromstring(byte, dtype=np.float64).reshape((blk_shape[0]*blk_shape[1]*blk_shape[2],3))
