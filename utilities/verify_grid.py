    ...
    for blk in mb_data:
        for face in blk.faces:
            ...
            if int(blk2.connectivity[nface2]['connection']) != blk.nblki:
                raise ValueError(f"Block {blk.nblki}'s' face {nface} says it is connected to\nblock {blk2.nblki}'s' face {nface2}, however block {blk2.nblki}'s\nface {nface2} says it is connected to a different block.")

            (face2_x, face2_y, face2_z) = extract_face(blk2, nface2)
            orientation2 = blk2.connectivity[nface2]['orientation']


            face_orientations = [i for j,i in enumerate(orientation) if j != int(face_to_orient_place_mapping[nface])]
            normal_index = [j for j in range(3) if j == int(face_to_orient_place_mapping[nface])][0]
            face_orientations2 = [i for j,i in enumerate(orientation2) if j != int(face_to_orient_place_mapping[nface2])]
            normal_index2 = [j for j in range(3) if j == int(face_to_orient_place_mapping[nface2])][0]
            ...
            try:
                ...
                raise ValueError(f'Error when comparing block {blk.nblki} and block {blk2.nblki} connection')

            if diff_x > 1e-14 and not periodic:
                print(f'Warning, the x coordinates of face {nface} on block {blk.nblki} are not matching the x coordinates of face {nface2} of block {blk2.nblki}')
                print(f'Off by average of {diff_x}')
                warn = True

            if diff_y > 1e-14 and not periodic:
                print(f'Warning, the y coordinates of face {nface} on block {blk.nblki} are not matching the y coordinates of face {nface2} of block {blk2.nblki}')
                print(f'Off by average of {diff_y}')
                warn = True

            if diff_z > 1e-14 and not periodic:
                print(f'Warning, the z coordinates of face {nface} on block {blk.nblki} are not matching the z coordinates of face {nface2} of block {blk2.nblki}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Verify grid is consistent in terms of connectivity and matching face coordinates.')
    parser.add_argument('-gpath', action='store', metavar='<grid_path>', dest='grid_path',
                        default='./', help='Path to grid files', type=str)
    parser.add_argument('-cpath', action='store', metavar='<conn_path>', dest='conn_path',
                        default='./', help='Path to conn.inp file', type=str)
