...
parser = argparse.ArgumentParser(description='Convert ICEM Multioblock-Info files into grid and connectivity files used by PEREGRINE')
parser.add_argument('-topo', action='store', metavar='<topo_FILE>', dest='topo_file_name',
                    default='info.topo', help='topology file from Multiblock-Info export from ICEM. Default is info.topo', type=str)
parser.add_argument('-fmt', action='store', metavar='<file_format>', dest='fmt',
                    default='tns3dmb', help='''Format used to export from ICEM **PAY ATTENTION TO THIS**. You must always export your ICEM grid in Multi-Block Info format to get the topo file. \nBut the
...
face_mapping = {'small_i':1, 'large_i':2,
                'small_j':3, 'large_j':4,
                'small_k':5, 'large_k':6}
orientation_mapping = { 'i':1,  'j':2,  'k':3,
                       '-i':4, '-j':5, '-k':6}
...
with open(args.topo_file_name, 'r') as f:
    ...
    for raw_line in iter_lines:
        ...
        if reading_block:
            tag = raw_line[0:2] # Only 'I ', 'E ', and 'S ' will be picked up as RAPTOR tags
...
with open(args.topo_file_name, 'r') as f:
    ...
    for raw_line in iter_lines:
        ...
        if raw_line.startswith('# Periodic info for domain'):
            ...
            if bc_type != 'f':
                raise ValueError(f'ERROR: we are expecting a face to be here to set face {face} of block{current_block}, but instead we are seeing a {bc_type}.')
            ...
            if this_block_line[2:5] == adjacent_block_line[2:5]:
                ...
                for i in range(3):
                    ...
                    face_num = f'small_{curr[-1]}' if curr.startswith('-') else f'large_{curr[-1]}'
    ...
    for raw_line in iter_lines:
        ...
        if raw_line.startswith('# Connectivity for domain'):
            ...
            for internal_face in internal_faces:
                ...
                for mini, maxi, direc in zip(mins,maxs,directions):
                    if mini == maxi and mini == '1':
                        ...
                        if str(face) != internal_face:
                            raise ValueError(f'Error, the order of the info.topo connectivity of block {current_block} is giving face {face} when we are expecting face {internal_face}')
                    elif mini == maxi and mini != '1':
                        face = face_mapping[f'large_{direc}']
                        if str(face) != internal_face:
                            raise ValueError(f'Error, the order of the info.topo connectivity of block {current_block} is giving face {face} when we are expecting face {internal_face}')
                ...
                adjacent_block_number = int(adjacent_block_line[1].strip().split('.')[-1])
                ...
                # Set the values
                blk.faces[internal_face].connectivity['neighbor'] = str(adjacent_block_number)
