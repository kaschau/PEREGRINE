...
import argparse
from peregrinepy.readers import *
...
if __name__ == '__main__':
    ...
    parser.add_argument('-from','--from_dir', action='store', metavar='<from_dir>', dest='from_dir',
                        default='./From', help='Directory containing the gv.*.h5 and q.*.h5 files to interpolate from. Default is ./From',type=str)
    parser.add_argument('-to','--to_dir', action='store', metavar='<to_dir>', dest='to_dir',
                        default='./To', help='Directory containing the gv.*.h5 files to interpolate to. Default is ./To',type=str)
    parser.add_argument('-ns','--ns', required=True, action='store', metavar='<number_of_sepecies>', dest='ns', help='Number of species in case', type=int)
    parser.add_argument('-func','--function', action='store', metavar='<function>', dest='function',
                        default='nearest', help='Interpolation type (nearest, linear, etc.)', type=str)
    parser.add_argument('-smooth','--smooth', action='store', metavar='<smooth>', dest='smooth',
                        default=0.5, help='Smoothing applied to interpolation ( larger is more smoothing ) Default = 0.5', type=float)
    parser.add_argument('-vs','--verbose-search', action='store_true', dest='verbose_search', help='If on, search will explicitly go through each block and be much slower, but the interpolation quality may improve expecially if you have a lot of curvy blocks.')
    ...
    from_nrst = int([i for i in os.listdir(from_dir) if i.startswith('q.')][0].strip().split('.')[1])
    ...
    if [] in bounds_list:
        raise ValueError('ERROR: It looks like there are blocks in your to-grid that are completely outside your from-grid domain')
